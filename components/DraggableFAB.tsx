...
export default function DraggableFAB({ fabOpen, setFabOpen, acc, onEvento, onCliente, onCommessa, onMessaggio, onLastCM, recentActions }) {
  ...
  const [liveMode, setLiveMode] = useState(false); // modalità conversazione live
  ...
  useEffect(() => {
    ...
    posRef.current = y; setTopPx(y);
  }, []);

  useEffect(() => {
    const onTouchStart = (e) => { dragRef.current = { on: true, moved: false, startY: e.touches[0].clientY, startTop: posRef.current }; e.preventDefault(); };
    const onTouchMove = (e) => {
      if (!dragRef.current.on) return;
      const dy = e.touches[0].clientY - dragRef.current.startY;
      if (Math.abs(dy) > 4) dragRef.current.moved = true;
      if (dragRef.current.moved && wrapRef.current) { const ny = Math.max(60, Math.min(window.innerHeight - 160, dragRef.current.startTop + dy)); wrapRef.current.style.top = ny + "px"; posRef.current = ny; }
      e.preventDefault();
    };
    const onTouchEnd = () => {
      if (!dragRef.current.on) return; dragRef.current.on = false;
      if (dragRef.current.moved) { setTopPx(posRef.current); localStorage.setItem("mastro:fab_top", String(posRef.current)); }
      else if (!fabOpenRef.current) setFabOpen(true);
    };
    ...
    return () => { tab.removeEventListener("touchstart", onTouchStart); window.removeEventListener("touchmove", onTouchMove); window.removeEventListener("touchend", onTouchEnd); };
  }, [setFabOpen]);

  // Sintesi vocale — AI risponde a voce
  const speak = (text) => {
    ...
    utt.onend = () => {
      setIsSpeaking(false);
      // In live mode: riavvia ascolto dopo risposta
      if (liveModeRef.current) setTimeout(() => startListening(), 300);
    };
  ...
  const sendAI = async (text) => {
    ...
    setAiLoading(true);
    try {
      const res = await fetch("/api/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: newMsgs, context: {} }) });
  ...
  // Attiva/disattiva live mode
  const toggleLiveMode = () => {
    if (liveMode) {
      stopListening(); stopSpeaking(); setLiveMode(false);
    } else {
      setLiveMode(true);
  ...
  const recent = (recentActions || []).slice(0, 3).map(ra => ({ l: ra.label, c: "#0D7C6B", t: "BCK", a: () => { try { const d = JSON.parse(ra.action); if (d.type === "commessa" && onLastCM) onLastCM({ id: d.id }); } catch {} } }));
  ...
  const AI  = ({s=20,c="#fff"}) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round"><path d="M12 2l2.4 7.2L22 12l-7.6 2.8L12 22l-2.4-7.2L2 12l7.6-2.8z"/></svg>;
  ...
  const MIC = ({c="#fff",s=20}) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round"><rect x="9" y="2" width="6" height="11" rx="3"/><path d="M5 10a7 7 0 0 0 14 0"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>;
  const SND = ({c="#fff"}) => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>;
  const SPK = ({c="#fff"}) => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>;
  const icons = { AI: () => <AI />, CAL, USR, FLD, MSG, BCK };

  return (
    <>
      {fabOpen && <div onClick={() => setFabOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(26,26,28,0.45)", zIndex: 89 }} />}
      {fabOpen && (
        <div style={{ position: "fixed", zIndex: 92, [isRight ? "right" : "left"]: 58, top: actionsTop, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div ref={wrapRef} style={{ position: "fixed", [isRight ? "right" : "left"]: 0, top: topPx, zIndex: 92 }}>
        <div style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ width: fabOpen ? 44 : 24, height: fabOpen ? 110 : 0, overflow: "hidden", background: "#0A5940", borderRadius: isRight ? "12px 0 0 0" : "0 12px 0 0", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, transition: "width 0.25s ease, height 0.25s ease" }}>
            <div onClick={() => setFabOpen(false)} style={{ cursor: "pointer" }}><span style={{ fontSize: 9, fontWeight: 800, color: "#fff", letterSpacing: 1 }}>CHIUDI</span></div>
          ...
          <div id="mastro-fab-tab" onClick={() => { if (!fabOpen) setFabOpen(true); }} style={{ width: fabOpen ? 44 : 24, height: 90, background: acc, borderRadius: isRight ? (fabOpen ? "0 0 0 12px" : "12px 0 0 12px") : (fabOpen ? "0 0 12px 0" : "0 12px 12px 0"), display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, cursor: "grab", userSelect: "none", WebkitUserSelect: "none", touchAction: "none", transition: "width 0.25s ease", boxShadow: isRight ? "-4px 0 20px " + acc + "60" : "4px 0 20px " + acc + "60" }}>
      ...
      {aiOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 999, display: "flex", flexDirection: "column", background: "#F8FAFC" }}>
          ...
          <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid #E2E8F0", background: "#fff", display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: acc, display: "flex", alignItems: "center", justifyContent: "center" }}><AI s={18} /></div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#0F172A" }}>MASTRO AI</div>
              <div style={{ fontSize: 11, color: "#64748B" }}>
                {liveMode ? (isListening ? "In ascolto..." : isSpeaking ? "Sto parlando..." : "Live — parla ora") : "Assistente intelligente"}
              </div>
            </div>
            {/* Live mode toggle */}
            <button onClick={toggleLiveMode} style={{ padding: "6px 12px", borderRadius: 20, border: `2px solid ${liveMode ? "#DC4444" : acc}`, background: liveMode ? "#DC4444" : "transparent", color: liveMode ? "#fff" : acc, fontSize: 11, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
              {liveMode ? <><SPK c="#fff" /> Stop</> : <><MIC c={acc} s={14} /> Live</>}
            </button>
          ...
          {/* Live mode indicator */}
          {liveMode && (
            <div style={{ background: isListening ? "#DC444415" : isSpeaking ? acc + "15" : "#F1F5F9", padding: "12px 20px", display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid #E2E8F0" }}>
              <div style={{ width: 44, height: 44, borderRadius: "50%", background: isListening ? "#DC4444" : isSpeaking ? acc : "#E2E8F0", display: "flex", alignItems: "center", justifyContent: "center", animation: isListening || isSpeaking ? "livepulse 1s ease-in-out infinite" : "none" }}>
                {isListening ? <MIC /> : isSpeaking ? <SPK /> : <MIC c="#94A3B8" />}
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#0F172A" }}>
                  {isListening ? "Sto ascoltando..." : isSpeaking ? "Sto rispondendo..." : "Parla per fare una domanda"}
                </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 0" }}>
            {aiMessages.length === 0 && !liveMode && (
              <div style={{ textAlign: "center", padding: "32px 20px" }}>
                <div style={{ marginBottom: 12, display: "flex", justifyContent: "center" }}><AI s={36} c={acc} /></div>
                <div style={{ fontSize: 15, fontWeight: 600, color: "#0F172A", marginBottom: 4 }}>Come posso aiutarti?</div>
                <div style={{ fontSize: 12, color: "#64748B", marginBottom: 16 }}>Scrivi, usa il microfono o premi <strong>Live</strong> per parlare liberamente</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
                  {["Quante commesse ho aperte?", "Fatture scadute?", "Riepilogo pipeline", "Chi devo chiamare oggi?", "Commesse ferme da 30 giorni"].map(s => (
            ...
            {aiMessages.map((msg, i) => (
              <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start", marginBottom: 12 }}>
                {msg.role === "assistant" && (
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: acc, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginRight: 8, marginTop: 2 }}><AI s={14} /></div>
                )}
            ...
            {aiLoading && (
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: acc, display: "flex", alignItems: "center", justifyContent: "center" }}><AI s={14} /></div>
                <div style={{ padding: "10px 16px", borderRadius: "16px 16px 16px 4px", background: "#fff", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                  <div style={{ display: "flex", gap: 4 }}>{[0,1,2].map(j => <div key={j} style={{ width: 6, height: 6, borderRadius: "50%", background: acc, animation: `fabpulse 1.2s ease-in-out ${j*0.2}s infinite` }} />)}</div>
                </div>
              </div>
            )}
      ...
      <style>{`
        @keyframes fabpulse { 0%,100%{opacity:0.3;transform:scale(0.8)} 50%{opacity:1;transform:scale(1)} }
