...
export default function MessaggiPanel() {
  const {
    ...
    const chCol = { email: #3b7fe0, whatsapp: "#25d366", sms: #e4c18c, telegram: "#0088cc" };
    ...
    return (
      <div style={{ paddingBottom: 80 }}>
        <div style={S.header}>
          ...
          <div onClick={() => setShowCompose(true)} style={{ width: 36, height: 36, borderRadius: 14, background: L.primary, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <Ico d={ICO.pen} s={16} c="#fff" />
          </div>
        </div>

        {/* Sub-tabs: Chat / Rubrica / AI */}
        <div style={{ display: "flex", margin: "8px 16px", borderRadius: 14, border: `1px solid ${L.border}`, overflow: "hidden" }}>
          {[
            ...
            <div key={st.id} onClick={() => setMsgSubTab(st.id)} style={{ flex: 1, padding: "10px 4px", textAlign: "center", fontSize: 12, fontWeight: 700, cursor: "pointer", background: msgSubTab === st.id ? L.primary : L.surface, color: msgSubTab === st.id ? "#fff" : L.sub, transition: "all 0.2s", position: "relative" }}>
              ...
              {st.count > 0 && <span style={{ marginLeft: 4, fontSize: 9, fontWeight: 800, padding: "1px 5px", borderRadius: 12, background: msgSubTab === st.id ? "rgba(255,255,255,0.3)" : L.red, color: "#fff" }}>{st.count}</span>}
            </div>
          ))}
        </div>
        ...
        {msgSubTab === "chat" && (<>
          <div style={{ padding: "4px 16px 8px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: L.surface, borderRadius: 14, border: `1px solid ${L.border}` }}>
          ...
          <div style={{ display: "flex", gap: 4, padding: "0 16px 10px", overflowX: "auto" }}>
            {[
              ...
              { id: "sms", l: "SMS", ico: ICO.phone, c: #e4c18c },
        ...
        {msgSubTab === "email" && (<>
          {!gmailStatus.connected ? (
            <div style={{ margin: "20px 16px", textAlign: "center" }}>
              ...
              <div onClick={() => window.location.href = "/api/gmail/auth"} style={{ display: "inline-block", padding: "14px 32px", borderRadius: 16, background: "#ea4335", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer" }}>
            ...
            <div style={{ padding: "0 16px" }}>
              ...
              {/* Email header */}
              <div style={{ background: L.surface, borderRadius: 16, padding: 14, marginBottom: 10, border: `1px solid ${L.border}` }}>
                ...
                {(() => { const match = gmailMatchCommessa(gmailSelected); return match ? (
                  <div onClick={() => { setSelectedCM(match); setTab("commesse"); }} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 12, background: L.primary + "10", border: `1px solid ${L.primary}30`, cursor: "pointer", marginTop: 4 }}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {gmailSelected.attachments.map((a, i) => (
                        <div key={i} style={{ padding: "4px 8px", borderRadius: 8, background: L.bg, border: `1px solid ${L.border}`, fontSize: 10, color: L.text }}>
              ...
              {/* Email body */}
              <div style={{ background: L.surface, borderRadius: 16, padding: 14, marginBottom: 10, border: `1px solid ${L.border}`, fontSize: 12, color: L.text, lineHeight: 1.7, whiteSpace: "pre-wrap", maxHeight: 300, overflowY: "auto" }}>
                {gmailSelected.body || gmailSelected.snippet || "(nessun contenuto)"}
              </div>
              {/* Quick actions */}
              <div style={{ background: L.surface, borderRadius: 16, padding: 12, marginBottom: 10, border: `1px solid ${L.border}` }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  {(() => { const match = gmailMatchCommessa(gmailSelected); return match ? (
                    <div onClick={() => { setSelectedCM(match); setTab("commesse"); }} style={{ padding: "8px 14px", borderRadius: 12, background: L.primary + "15", color: L.primary, fontSize: 11, fontWeight: 700, cursor: "pointer" }}><I d={ICO.folder} /> Apri {match.code}</div>
                  ) : null; })()}
                  {/* Crea nuova commessa */}
                  <div onClick={() => {
                    ...
                    setTab("commesse");
                  }} style={{ padding: "8px 14px", borderRadius: 12, background: "#34c75915", color: "#34c759", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                  ...
                  <div onClick={() => {
                    ...
                    setGmailSelected(null);
                    setTab("agenda");
                  }} style={{ padding: "8px 14px", borderRadius: 12, background: "#5856d615", color: "#5856d6", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                  ...
                  <div onClick={() => {
                    ...
                    setTab("agenda");
                    setGmailSelected(null);
                  }} style={{ padding: "8px 14px", borderRadius: 12, background: "#ff950015", color: "#ff9500", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                  ...
                  }} style={{ padding: "8px 14px", borderRadius: 12, background: "#af52de15", color: "#af52de", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                    <I d={ICO.users} /> Rubrica
                  </div>
                  {/* Copia email */}
                  <div onClick={() => { const fromEmail = gmailSelected.from?.match(/<(.+)>/)?.[1] || gmailSelected.from; navigator.clipboard?.writeText(fromEmail); alert("Email copiata!"); }} style={{ padding: "8px 14px", borderRadius: 12, background: L.bg, color: L.sub, fontSize: 11, fontWeight: 700, cursor: "pointer", border: `1px solid ${L.border}` }}><I d={ICO.clipboard} /> Copia email</div>
                  {/* Seleziona tutto il testo */}
                  <div onClick={() => { navigator.clipboard?.writeText(gmailSelected.body || gmailSelected.snippet || ""); alert("Testo email copiato!"); }} style={{ padding: "8px 14px", borderRadius: 12, background: L.bg, color: L.sub, fontSize: 11, fontWeight: 700, cursor: "pointer", border: `1px solid ${L.border}` }}><I d={ICO.fileText} /> Copia testo</div>
                </div>
              </div>
              {/* Reply */}
              <div style={{ background: L.surface, borderRadius: 16, padding: 12, border: `1px solid ${L.border}` }}>
                ...
                <textarea value={gmailReply} onChange={e => setGmailReply(e.target.value)} rows={4} placeholder="Scrivi la risposta..."
                  style={{ width: "100%", padding: 10, borderRadius: 12, border: `1px solid ${L.border}`, background: L.bg, fontSize: 12, color: L.text, fontFamily: "inherit", resize: "vertical", boxSizing: "border-box" as any, lineHeight: 1.5 }} />
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  ...
                  }} style={{ flex: 1, padding: 12, borderRadius: 14, border: "none", background: gmailReply.trim() ? "#007aff" : L.border, color: "#fff", fontSize: 13, fontWeight: 700, cursor: gmailReply.trim() ? "pointer" : "default", fontFamily: "inherit", opacity: gmailSending ? 0.6 : 1 }}>
            ...
            <div style={{ padding: "0 16px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: L.surface, borderRadius: 14, border: `1px solid ${L.border}` }}>
                ...
                <div onClick={() => gmailFetchMessages(gmailSearch)} style={{ padding: "8px 12px", borderRadius: 14, background: L.primary, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                  <I d={ICO.refreshCw} />
                </div>
              </div>
              ...
              {gmailLoading && gmailMessages.length === 0 ? (
                ...
                <>
                  {gmailMessages.map(m => {
                    ...
                    return (
                      <div key={m.id} onClick={() => setGmailSelected(m)} style={{ padding: "10px 12px", background: L.surface, borderRadius: 14, border: `1px solid ${m.unread ? L.primary + "40" : L.border}`, marginBottom: 6, cursor: "pointer", borderLeft: m.unread ? `3px solid ${L.primary}` : "none" }}>
        ...
        {msgSubTab === "ai" && (<>
          {/* Header spiegazione */}
          <div style={{ margin: "0 16px 10px", padding: "10px 12px", borderRadius: 14, background: "linear-gradient(135deg, #1A1A1C, #2A2008)", border: `1px solid ${L.primary}30`, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div key={m.id} style={{ ...S.card, marginBottom: 8, padding: 0, overflow: "hidden", opacity: m.archiviata ? 0.5 : 1 }}>
                  ...
                  <div onClick={() => setSelectedAiMsg(isSelected ? null : m)}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 4, marginTop: 4, alignItems: "center", flexWrap: "wrap" }}>
                        <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 12, background: m.ai.color + "18", color: m.ai.color, border: `1px solid ${m.ai.color}30` }}>
                  ...
                  {isSelected && (
                    <div style={{ borderTop: `1px solid ${L.border}`, padding: "12px 13px" }}>
                      {/* Testo email */}
                      <div style={{ fontSize: 11, color: L.sub, lineHeight: 1.6, marginBottom: 10, padding: "8px 10px", background: L.bg, borderRadius: 12 }}>
                        {m.body}
                      </div>

                      {/* Analisi AI */}
                      <div style={{ background: m.ai.color + "10", border: `1px solid ${m.ai.color}30`, borderRadius: 14, padding: "10px 12px", marginBottom: 10 }}>
                      ...
                      {m.ai.estratto && (
                        <div style={{ background: #d1fae5, border: `1px solid ${L.green}30`, borderRadius: 14, padding: "10px 12px", marginBottom: 10 }}>
...
`;
                      ...
                      {m.archiviata && (
                        <div style={{ padding: "8px 12px", background: #d1fae5, borderRadius: 12, textAlign: "center", fontSize: 12, fontWeight: 700, color: L.green }}>
