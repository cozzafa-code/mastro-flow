...
function MastroMisureInner({ user, azienda: aziendaInit }: { user?: any, azienda?: any }) {
  ...
  // NO DEMO DATA ÔÇö production mode, all empty by default
  ...
  // Offline cache IndexedDB ÔÇö carica al mount, salva ad ogni cambio
  ...
  const [ctSezioniDB, setCtSezioniDB] = useState([
    { id: "cs1", code: "56├âÔÇö40" }, { id: "cs2", code: "56├âÔÇö50" }, { id: "cs3", code: "56├âÔÇö60" }, { id: "cs4", code: "56├âÔÇö70" }, { id: "cs5", code: "76├âÔÇö50" }, { id: "cs6", code: "76├âÔÇö60" }
  ]);
  ...
  const [fornitori, setFornitori] = useState(() => {
    ...
    { id: "f2", nome: "Sch├â┬╝co International", ragioneSociale: "Sch├â┬╝co International Italia SRL", piva: "09876543210", cf: "", tipo: "Profili Alluminio", categoria: "profili", indirizzo: "Via Milano 50", cap: "20100", citta: "Milano", provincia: "MI", telefono: "+39 02 654321", cellulare: "", email: "italia@schueco.com", pec: "schuco@pec.it", sito: "www.schueco.com", referente: "Luca Bianchi", telReferente: "+39 335 7654321", emailReferente: "l.bianchi@schueco.com", banca: "Intesa", iban: "", pagamento: "60gg_fm", scontoBase: 30, tempoConsegna: 22, sistemiTrattati: "AWS 75, ASS 70, ASS 77", note: "", rating: 4.8, preferito: true, attivo: true },
  ...
  const calcolaVanoPrezzo = (v, c) => {
    ...
    // Helper: cerca prezzo in listino settore (griglia L├âÔÇöH o ├óÔÇÜ┬¼/mq con minimo)
    const getListinoPrice = (listino: any[], lmm: number, hmm: number) => {
      if (!listino?.length) return null;
      const mq = (lmm / 1000) * (hmm / 1000);
      // Prova griglia L├âÔÇöH per ogni prodotto del listino (usa il primo match)
  ...
  // DELETE functions
  const deleteTask = (taskId) => { confirm({ title: "Eliminare task?", message: "Questa azione ├¿ irreversibile.", confirmText: "Elimina", danger: true, onConfirm: () => { setTasks(ts => ts.filter(t => t.id !== taskId)); toast("Task eliminato", "success"); } }); };
  ...
  const deleteCommessa = (cmId) => {
    ...
    confirm({
      ...
      message: `Stai per eliminare "${cm?.cliente || "commessa"}" e tutti i suoi vani, rilievi e documenti. Questa azione ├¿ irreversibile.`,
      confirmText: "Elimina",
      danger: true,
      onConfirm: () => {
  ...
  const deleteEvent = (evId) => { confirm({ title: "Eliminare evento?", message: "L'evento verr├á rimosso dal calendario.", confirmText: "Elimina", danger: true, onConfirm: () => { setEvents(ev => ev.filter(e => e.id !== evId)); toast("Evento eliminato", "success"); } }); };
  const deleteMsg = (msgId) => { confirm({ title: "Eliminare messaggio?", message: "Il messaggio verr├á rimosso.", confirmText: "Elimina", danger: true, onConfirm: () => { setMsgs(ms => ms.filter(m => m.id !== msgId)); toast("Messaggio eliminato", "success"); } }); };
  ...
  const startMediaRecording = async (tipo: "vocale" | "video") => {
    try {
      ...
      alert("├»┬©┬Å Impossibile accedere al " + (tipo === "video" ? "camera/microfono" : "microfono") + ". Controlla i permessi del browser.\n\n" + (err as Error).message);
    }
  };
  ...
  const openCamera = async (mode: "foto" | "video", cat?: string) => {
    ...
    try {
      ...
      alert("├»┬©┬Å Impossibile accedere alla fotocamera. Controlla i permessi.\n\n" + (err as Error).message);
  ...
  const addSettingsItem = () => {
    ...
    if (settingsModal === "sistema" && f.marca && f.sistema) {
      ...
      TIPOLOGIE_RAPIDE.push({ code: f.code, label: f.label, icon: f.icon || "├ó┼á┼¥", cat: f.cat || "Altro", forma: f.forma || "rettangolare" });
  ...
  const deleteSettingsItem = (type, id) => {
    confirm({ title: "Eliminare elemento?", message: "L'elemento verr├á rimosso dal catalogo.", confirmText: "Elimina", danger: true, onConfirm: () => {
  ...
  const setFaseTo = (cmId: string, targetFase: string, operatore?: string) => {
    ...
    if (bloccato) {
      ...
      if (c) {
        ...
        toast(`├óÔÇ║ÔÇØ Gate bloccante - ${mancanti.join(" ├é┬À ")}`, "error");
      }
      return;
    }

    // Aggiorna selectedCM se ├¿ la commessa aperta
    if (selectedCM?.id === cmId) {
  ...
  const handleAI = () => {
    ...
    if (q.includes("oggi") || q.includes("programma")) {
      ...
      resp = `Hai ${cantieri.length} commesse:\n${cantieri.map(c => `├óÔé¼┬ó ${c.code} ${c.cliente} - ${PIPELINE.find(p => p.id === c.fase)?.nome}`).join("\n")}`;
    } else if (q.includes("vani") || q.includes("misur")) {
      resp = `Totale vani: ${countVani()}\nCommesse con vani da misurare:\n${cantieri.filter(c => getVaniAttivi(c).some(v => Object.keys(v.misure || {}).length < 6)).map(c => `├óÔé¼┬ó ${c.code}: ${getVaniAttivi(c).filter(v => Object.keys(v.misure || {}).length < 6).length} vani incompleti`).join("\n")}`;
    } else if (q.includes("urgent") || q.includes("priorit")) {
      const u = tasks.filter(x => x.priority === "alta" && !x.done);
      resp = u.length ? `Task urgenti:\n${u.map(x => `├óÔé¼┬ó ${x.text}`).join("\n")}` : "Nessun task urgente!";
    }
  ...
  const exportPDF = () => {
    ...
    vaniExport.forEach((v, i) => {
      ...
      html += `</div>`;
      if (v.cassonetto) html += `<p style="margin-top:8px;font-size:13px">Cassonetto${v.casTipo ? " " + v.casTipo : ""}: ${(v.misure||{}).casL || " - "}├âÔÇö${(v.misure||{}).casH || " - "}├âÔÇö${(v.misure||{}).casP || " - "} mm</p>`;
  ...
  const SectionHead = ({ id, icon, title, count, countColor, extra }: { id: string; icon: string; title: string; count?: number; countColor?: string; extra?: any }) => (
    <div style={{ ...S.section, margin: "0 16px" }}>
      <div onClick={() => toggleCollapse(id)} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", flex: 1 }}>
        ...
        <span style={{ fontSize: 8, color: T.sub, marginLeft: 2, transform: collapsed[id] ? "rotate(-90deg)" : "rotate(0deg)", transition: "transform 0.15s", display: "inline-block" }}>├óÔÇô┬╝</span>
      </div>
      {extra}
    </div>
  );
  ...
  const renderFasePanel = (c) => {
      ...
      return (
        <div style={panelStyle}>
          <div onClick={() => togglePanel(id)} style={{ ...headerStyle, cursor:"pointer",
            ...
            <span style={{fontSize:13,color:T.sub,transform:isOpen(id)?"rotate(0deg)":"rotate(-90deg)",transition:"transform 0.2s",flexShrink:0}}>├óÔÇô┬¥</span>
          </div>
          {isOpen(id) && (
      ...
      return (
        <div style={panelStyle}>
          <div onClick={()=>togglePanel("sopralluogo")} style={{...headerStyle,cursor:"pointer",borderBottom:open_sopr?`1px solid ${fase?.color}25`:"none",userSelect:"none"}}>
            ...
            <span style={{fontSize:13,color:T.sub,transform:open_sopr?"rotate(0deg)":"rotate(-90deg)",transition:"transform 0.2s"}}>├óÔÇô┬¥</span>
          </div>
          {open_sopr && <div style={{padding:"12px 14px"}}>
            <Chip label="Fotografie scattate" done={c.ck_foto} onClick={()=>updateCM("ck_foto",!c.ck_foto)}/>
            <Chip label="Difficolt├á accesso rilevata" done={c.ck_accesso} onClick={()=>updateCM("ck_accesso",!c.ck_accesso)}/>
    ...
    if (c.fase === "preventivo") {
      ...
      return (
        <div style={panelStyle}>
          ...
          <div style={{padding:"12px 14px"}}>
            <Field label="Prezzo base ├óÔÇÜ┬¼/mq" field="prezzoMq" placeholder="350" type="number"/>
            ...
            <div style={{padding:"10px 12px",borderRadius:8,background:T.bg,border:`1px solid ${T.bdr}`,marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:T.sub,marginBottom:4}}>
                <span>Totale imponibile</span><span style={{fontWeight:700,color:T.text}}>├óÔÇÜ┬¼ {totale.toFixed(2)}</span>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:T.sub,marginBottom:4}}>
                <span>IVA 10%</span><span>├óÔÇÜ┬¼ {iva.toFixed(2)}</span>
              </div>
              {c.sconto > 0 && (
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:T.orange,marginBottom:4}}>
                  <span>Sconto {c.sconto}%</span><span>- ├óÔÇÜ┬¼ {(totale * c.sconto/100).toFixed(2)}</span>
                </div>
              )}
              <div style={{borderTop:`1px solid ${T.bdr}`,marginTop:6,paddingTop:6,display:"flex",justifyContent:"space-between",fontSize:14,fontWeight:800}}>
                ...
                <span style={{color:T.acc}}>├óÔÇÜ┬¼ {(totale + iva - (totale*(c.sconto||0)/100)).toFixed(2)}</span>
              </div>
            </div>
    ...
    if (c.fase === "conferma") {
      return (
        <div style={panelStyle}>
          <div style={headerStyle}>
            <span style={{fontSize:16}}><I d={ICO.edit} />├»┬©┬Å</span>
          ...
          <div style={{padding:"12px 14px"}}>
            ...
            <Field label="Acconto ricevuto ├óÔÇÜ┬¼" field="accontoRicevuto" placeholder="0" type="number"/>
          ...
          return (
            <div style={panelStyle}>
              <div onClick={()=>togglePanel("misure")} style={{...headerStyle,cursor:"pointer",borderBottom:open?`1px solid ${fase?.color}25`:"none",userSelect:"none"}}>
                ...
                {ndone>0 && <span style={{width:8,height:8,borderRadius:"50%",background:T.red,display:"inline-block",marginRight:6}}/>}
                <span style={{fontSize:13,color:T.sub,transform:open?"rotate(0deg)":"rotate(-90deg)",transition:"transform 0.2s"}}>├óÔÇô┬¥</span>
              </div>
        ...
        (() => {
          ...
          return (
            <div style={panelStyle}>
              ...
              {open && <div style={{padding:"12px 14px"}}>
                <Field label="Fornitore" field="fornitore" placeholder="Es. Sch├â┬╝co, Rehau..."/>
                <Field label="N├é┬░ Ordine fornitore" field="numOrdine" placeholder="ORD-2026-XXXX"/>
          ...
          <div style={{padding:"12px 14px"}}>
            <div style={{padding:"10px 12px",borderRadius:8,background:T.bg,border:`1px solid ${T.bdr}`,marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}>
                <span style={{color:T.sub}}>Totale commessa</span><span style={{fontWeight:700}}>├óÔÇÜ┬¼ {totIva.toFixed(2)}</span>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}>
                <span style={{color:T.sub}}>Acconto ricevuto</span><span style={{color:T.grn,fontWeight:700}}>- ├óÔÇÜ┬¼ {parseFloat(c.accontoRicevuto||0).toFixed(2)}</span>
              </div>
              <div style={{borderTop:`1px solid ${T.bdr}`,paddingTop:6,marginTop:2,display:"flex",justifyContent:"space-between",fontSize:15,fontWeight:800}}>
                <span>Saldo da incassare</span><span style={{color:saldo>0?T.red:T.grn}}>├óÔÇÜ┬¼ {saldo.toFixed(2)}</span>
              </div>
            </div>
            <Field label="Data chiusura" field="dataChiusura" type="date"/>
            <Field label="Saldo incassato ├óÔÇÜ┬¼" field="saldoIncassato" placeholder="0" type="number"/>
            ...
            {c.ck_saldo && c.ck_fattura && (
              <div style={{marginTop:8,padding:"12px",borderRadius:8,background:T.grn+"15",border:`1px solid ${T.grn}30`,textAlign:"center"}}>
                ...
                <div style={{fontSize:11,color:T.sub,marginTop:2}}>{c.code} ├é┬À {c.cliente} {c.cognome||""}</div>
              </div>
            )}
          </div>
        </div>
                ...
                {clCM.map((c: any) => (
                  <div key={c.id} onClick={() => { setSelectedCliente(null); setSelectedCM(c); setTab("commesse"); }}
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 700, color: T.acc, background: T.accLt, padding: "3px 8px", borderRadius: 6 }}>{c.fase || "ÔÇö"}</span>
                    </div>
  ...
  const creaOrdineFornitore = (c, fornitoreNome = "") => {
    ...
    const righe = vani.map(v => {
      ...
      return {
        ...
        misure: lmm > 0 && hmm > 0 ? `${lmm}├âÔÇö${hmm}` : "da definire",
        qta: v.pezzi || 1,
    ...
    return (
      <div style={{ background: T.card, borderRadius: 12, border: `1px solid ${T.bdr}`, overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", background: T.bg, borderBottom: `1px solid ${T.bdr}` }}>
          <div onClick={() => setCalMontaggiWeek(w => w - 1)} style={{ padding: "4px 10px", cursor: "pointer", fontSize: 16, fontWeight: 700, color: T.acc }}>├óÔÇöÔé¼</div>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div onClick={() => setCalMontaggiWeek(w => w + 1)} style={{ padding: "4px 10px", cursor: "pointer", fontSize: 16, fontWeight: 700, color: T.acc }}>├óÔÇô┬Â</div>
          </div>
        </div>
                  ...
                  <div key={i} onClick={() => {
                    ...
                    {occ && (
                      <div style={{ fontSize: 7, fontWeight: 700, color: sq.colore, lineHeight: 1.2 }}>
                        ...
                        <div style={{ color: T.sub }}>{(occ as any).vaniCount}v ├é┬À {(occ as any).durata === "mezza" ? "├é┬¢" : (occ as any).durata === "2giorni" ? "2g" : (occ as any).durata === "3giorni" ? "3g" : "1g"}</div>
                      </div>
                    )}
                    {canClick && !occ && (
              ...
              {weekDeliveries.map(o => {
                ...
                return (
                  <div key={o.id} style={{ fontSize: 10, color: isLate ? T.red : T.text, padding: "2px 0", display: "flex", gap: 8 }}>
                    ...
                    {o.costo > 0 && <span style={{ color: T.sub }}>├óÔÇÜ┬¼{o.costo.toLocaleString("it-IT")}</span>}
                    {isLate && <span style={{ color: T.red, fontWeight: 700 }}><I d={ICO.alertTriangle} />├»┬©┬Å RITARDO</span>}
                  </div>
                );
              })}
            </div>
    ...
    <>
      <div style={S.app}>
        ...
        {selectedTask && (() => {
          ...
          const prioLabel = t.priority === "alta" ? "Urgente" : t.priority === "media" ? "Normale" : "├óÔÇö┬Å Bassa";
          return (
            <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setSelectedTask(null)}>
              ...
              <div onClick={(e) => e.stopPropagation()} style={{ position: "relative", zIndex: 9999, background: T.bg, borderRadius: 16, padding: 20, width: "90%", maxWidth: 420, boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 9, fontWeight: 800, color: prioColor, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4, fontFamily: FM }}>TASK ├é┬À {prioLabel}</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <div onClick={() => { toggleTask(t.id); setSelectedTask({ ...t, done: !t.done }); }} style={{ padding: "14px", borderRadius: 12, background: t.done ? T.bg : T.grn, color: t.done ? T.sub : "#fff", textAlign: "center", cursor: "pointer", fontSize: 14, fontWeight: 800, border: `1px solid ${t.done ? T.bdr : T.grn}` }}>{t.done ? "├óÔÇá┬® Riapri" : "Completa"}</div>
          ...
          <div style={{ position: "fixed", inset: 0, background: T.bg, zIndex: 150, display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "12px 16px", background: T.card, borderBottom: `1px solid ${T.bdr}`, display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: T.sub }}>{selectedMsg.cm ? `${selectedMsg.cm} ├é┬À ` : ""}{selectedMsg.thread?.length || 0} messaggi</div>
              </div>
              {selectedMsg.cm && (
            ...
            <div style={{ padding: "6px 16px", background: T.bg, borderBottom: "1px solid " + T.bdr, display: "flex", gap: 6, overflowX: "auto" }}>
              ...
              <div onClick={() => {
                ...
                if (exists) { toast(fromName + " gi├á in rubrica", "warning"); return; }
              ...
              {(selectedMsg.thread || []).map((msg, i) => {
                ...
                return (
                  <div key={i} style={{ marginBottom: 12, display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start" }}>
                    <div style={{ fontSize: 9, color: T.sub, marginBottom: 3, fontWeight: 600 }}>{mChIco} {msg.who} ├é┬À {msg.date} {msg.time}</div>
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={e => e.target === e.currentTarget && setSettingsModal(null)}>
            <div style={{ background: T.card, borderRadius: 16, width: "100%", maxWidth: 380, padding: 20 }}>
              ...
              {settingsModal === "sistema" && (<>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <div style={{ flex: 1 }}><label style={S.fieldLabel}>├óÔÇÜ┬¼/mq (fallback)</label><input style={S.input} type="number" placeholder="180" value={settingsForm.euroMq || ""} onChange={e => setSettingsForm(f => ({ ...f, euroMq: e.target.value }))} /></div>
              ...
              {settingsModal === "vetro" && (<>
                ...
                <div style={{ marginBottom: 10 }}><label style={S.fieldLabel}>Prezzo ├óÔÇÜ┬¼/mq</label><input style={S.input} type="number" step="0.5" placeholder="es. 45" value={settingsForm.prezzoMq || ""} onChange={e => setSettingsForm(f => ({ ...f, prezzoMq: parseFloat(e.target.value)||0 }))} /></div>
              </>)}
              ...
              {(settingsModal === "coprifilo" || settingsModal === "lamiera") && (<>
                ...
                <div style={{ marginBottom: 10 }}><label style={S.fieldLabel}>Prezzo ├óÔÇÜ┬¼/ml</label><input style={S.input} type="number" step="0.5" placeholder="es. 5.50" value={settingsForm.prezzoMl || ""} onChange={e => setSettingsForm(f => ({ ...f, prezzoMl: parseFloat(e.target.value)||0 }))} /></div>
              </>)}
              ...
              {settingsModal === "tipologia" && (<>
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  ...
                  <div style={{ width: 60 }}><label style={S.fieldLabel}>Icona</label><input style={S.input} placeholder="├ó┼á┼¥" value={settingsForm.icon || ""} onChange={e => setSettingsForm(f => ({ ...f, icon: e.target.value }))} /></div>
                </div>
          ...
          return (
            <div style={S.modal} onClick={e => e.target === e.currentTarget && setShowProblemaModal(false)}>
              <div style={{ ...S.modalInner, maxHeight: "85vh", overflow: "auto" }}>
                ...
                <div style={{ fontSize: 11, color: T.sub, marginBottom: 14, padding: "8px 12px", background: T.accLt, borderRadius: 8 }}><Ico d={ICO.folder} s={12} c={T.acc} /> {c.code} ├é┬À {c.cliente} ├é┬À Fase: <b>{c.fase}</b></div>
                ...
                <label style={S.fieldLabel}>Priorit├á</label>
        ...
        {showProblemiView && (() => {
          ...
          const TIPI_PROB_MAP = { materiale: { l: "Materiale", ico: "package", c: "#FF9500" }, misure: { l: "Misure", ico: "ruler", c: "#8B5CF6" }, installazione: { l: "Install.", ico: "hammer", c: "#1A9E73" }, cliente: { l: "Cliente", ico: "user", c: "#0D7C6B" }, fornitore: { l: "Fornitore", ico: "factory", c: "#FF6B00" }, qualita: { l: "Qualit├á", ico: "alert", c: "#FF3B30" }, altro: { l: "Altro", ico: "clipboard", c: "#8E8E93" } };
            ...
            <div style={S.modal} onClick={e => e.target === e.currentTarget && setShowProblemiView(false)}>
              <div style={{ ...S.modalInner, maxWidth: 500, maxHeight: "90vh", overflow: "auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <div style={{ fontSize: 16, fontWeight: 800, color: T.text }}><I d={ICO.alertTriangle} /> Problemi {cmFilter ? `├é┬À ${selectedCM.code}` : " -  Tutti"}</div>
                ...
                {list.length === 0 ? (
                  ...
                  const prio = p.priorita === "alta" ? { l: "", c: "#FF3B30" } : p.priorita === "media" ? { l: "", c: "#FF9500" } : { l: "├óÔÇö┬Å", c: "#8E8E93" };
                  return (
                    <div key={p.id} style={{ background: T.card, borderRadius: 12, border: `1px solid ${T.bdr}`, padding: "12px 14px", marginBottom: 8, borderLeft: `3px solid ${st.c}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 11, color: T.sub, marginTop: 2 }}>{p.commessaCode} ├é┬À {p.cliente} ├é┬À {p.fase}</div>
                        </div>
      ...
      {/* === TUTORIAL INTERATTIVO === */}
            {/* ├óÔÇó┬É├óÔÇó┬É├óÔÇó┬É ONBOARDING 5 STEP ├óÔÇó┬É├óÔÇó┬É├óÔÇó┬É */}
            ...
            <div style={{ flex: 1, padding: "24px 20px", maxWidth: 480, margin: "0 auto", width: "100%", boxSizing: "border-box" as any }}>
              ...
              {tutoStep === 3 && (
                <div>
                  ...
                  <div style={{ fontSize: 13, color: "#666", marginBottom: 24 }}>Seleziona uno o pi├╣ settori. Puoi cambiare in qualsiasi momento.</div>
              ...
              {/* STEP 4 PREZZI ÔÇö RIMOSSO */}
{/* STEP 4 ÔÇö TUTTO PRONTO (renamed) - TUTTO PRONTO */}
              {tutoStep === 4 && (
                <div style={{ textAlign: "center" }}>
                  ...
                  <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 28, textAlign: "left" }}>
                    {[
                      ...
                      { n: "4", t: "Invia il preventivo", d: "D├¼ Mastro, manda il preventivo a [cliente] e il gioco ├¿ fatto" },
                    ].map((s, i) => (
          ...
          <div style={S.modal} onClick={e => e.target === e.currentTarget && setShowNewEvent(false)}>
            <div style={S.modalInner}>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={S.fieldLabel}>Priorit├á</label>
                <div style={{ display: "flex", gap: 6 }}>
                  {[{ id: "alta", l: "Alta", c: "#FF3B30" }, { id: "media", l: "Media", c: "#FF9500" }, { id: "bassa", l: "├óÔÇö┬Å Bassa", c: "#8E8E93" }].map(p => (
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <select style={S.select} value={newEvent.cm} onChange={e => setNewEvent(ev => ({ ...ev, cm: e.target.value }))}>
                  <option value=""> -  Nessuna  - </option>
                  {cantieri.map(c => <option key={c.id} value={c.code}>{c.code} ├é┬À {c.cliente}</option>)}
                </select>
              </div>
              <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {newEvent.reminder && (
                  <div style={{ marginTop: 6, fontSize: 10, color: T.sub, padding: "5px 8px", background: T.accLt, borderRadius: 6 }}>
                    <I d={ICO.mail} /> MASTRO ti avviser├á di inviare il reminder - lo farai con 1 click dal banner in agenda
                  </div>
                )}
              </div>
              </>)}
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {[
                    ...
                    { id: "telegram", l: "├»┬©┬Å Telegram", c: "#0088cc" },
                  ].map(ch => (
            ...
            <div style={{ background: T.card, borderRadius: 16, width: "100%", maxWidth: 380, padding: 20, maxHeight: "80vh", overflowY: "auto" }}>
              {aiPhotoStep === 0 && (
                <>
                  <div style={{ textAlign: "center", marginBottom: 16 }}>
                    ...
                    <div style={{ fontSize: 12, color: T.sub, marginTop: 4 }}>Inquadra il vano "{selectedVano?.nome}" e l'AI analizzer├á l'immagine</div>
                  </div>
              ...
              {aiPhotoStep === 1 && (
                <div style={{ textAlign: "center", padding: "40px 0" }}>
                  ...
                  <div style={{ fontSize: 11, color: T.sub, marginTop: 4 }}>Rilevamento bordi ├é┬À Edge detection ├é┬À Stima dimensioni</div>
              ...
              {aiPhotoStep === 2 && (
                <>
                  ...
                  <div style={{ padding: "8px 12px", borderRadius: 8, background: "#fff3e0", border: "1px solid #ffe0b2", marginBottom: 12, fontSize: 10, color: "#e65100" }}>
                    <I d={ICO.alertTriangle} />├»┬©┬Å Le misure AI sono approssimative. Usa sempre il metro laser per le misure definitive.
                  </div>
                  <button onClick={() => {
                    if (selectedVano) {
      ...
      {showContabilita && <PanelErrorBoundary name="Contabilit├á">{renderContabilita()}</PanelErrorBoundary>}
        {showInboxDoc && (
              ...
              <div>
                ...
                {inboxResult.dati && (inboxResult.dati.fornitoreNome || inboxResult.dati.totale > 0 || inboxResult.dati.settimane > 0) && (
                  <div style={{ ...S.card, padding: 12, marginBottom: 12, background: "#af52de08", border: `1px solid #af52de20` }}>
                    ...
                    {inboxResult.dati.totale > 0 && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "3px 0" }}><span style={{ color: T.sub }}>Totale</span><b style={{ color: "#af52de" }}>├óÔÇÜ┬¼{inboxResult.dati.totale.toLocaleString("it-IT", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b></div>}
                ...
                {inboxResult.docTipo === "conferma" && inboxResult.matchedOrdine && (
                  <div style={{ ...S.card, padding: 12, marginBottom: 12, background: "#1A9E7308", border: `2px solid #1A9E73` }}>
                    ...
                    <div style={{ fontSize: 11, color: T.sub, marginTop: 2 }}>{inboxResult.matchedOrdine.fornitore?.nome || " - "} ├é┬À ├óÔÇÜ┬¼{(inboxResult.matchedOrdine.totaleIva || 0).toLocaleString("it-IT")}</div>
                    ...
                    {inboxResult.tuttiOrdini.filter(o => o.id !== inboxResult.matchedOrdine?.id).map(o => {
                      ...
                      return (
                        <div key={o.id} onClick={() => confermaInboxDoc(o.id)} style={{ ...S.card, padding: "10px 12px", marginBottom: 6, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", border: `1px solid ${T.bdr}` }}>
                          ...
                          <div style={{ fontSize: 11, fontWeight: 700, color: T.acc }}>├óÔÇÜ┬¼{(o.totaleIva || 0).toLocaleString("it-IT")}</div>
                        </div>
                      );
                    })}
                  </div>
                    ...
                    {(inboxResult.commesseAttive || cantieri).filter(cm => cm.id !== inboxResult.matchedCommessa?.id).map(cm => (
                      <div key={cm.id} onClick={() => assegnaDocUniversale(cm.id, inboxResult.docTipo)} style={{ ...S.card, padding: "10px 12px", marginBottom: 6, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", border: `1px solid ${T.bdr}` }}>
                        <div>
                          ...
                          <div style={{ fontSize: 10, color: T.sub }}>{cm.fase} ├é┬À {cm.indirizzo || " - "}</div>
                        </div>
                        <span style={{ fontSize: 16 }}> ></span>
                      </div>
                    ))}
                ...
                <div style={{ marginTop: 12, padding: 10, borderRadius: 10, background: T.bg, textAlign: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 4, justifyContent: "center", flexWrap: "wrap" as any }}>
                    {[
                      { id: "firma", label: "├»┬©┬Å Firma", col: "#1A9E73" },
            ...
            {/* All attachments */}
                ...
                <div style={{ marginTop: 12 }}>
                  <div onClick={() => setShowCronologia(!showCronologia)} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", padding: "8px 0" }}>
                    ...
                    <span style={{ marginLeft: "auto", fontSize: 10, color: T.sub }}>{showCronologia ? "├óÔÇô┬▓" : "├óÔÇô┬╝"}</span>
                  </div>
