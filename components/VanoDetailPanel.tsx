        ...
        <div style={{ position: "fixed", inset: 0, background: "#F4F1EA", zIndex: 9900, overflow: "auto", fontFamily: FF }}>
          ...
          <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
            {(selectedRilievo.vani||[]).map((vr: any, idx: number) => {
              ...
              const Row = (label: string, value: any, k: string) => (
              ...
              const Sez = (title: string, rows: any[], sk: string) => (
                <div key={sk} style={{ marginTop: 10, padding: "10px 12px", background: "#FAFAF5", borderRadius: 10, border: "1px solid #F0EDE5" }}>
                  ...
                  {rows.map((r: any, i: number) => Row(r[0], r[1], sk + "_" + i))}
                </div>
              );
              return (
                <div key={vr.id} style={{ background: "#fff", borderRadius: 14, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                  ...
                  {Sez("TIPOLOGIA", [["Tipo", vr.tipo], ["Stanza", vr.stanza], ["Piano", vr.piano], ["Pezzi", vr.pezzi]], "tip" + idx)}
                  {Sez("SISTEMA", [["Profilo", vr.sistema], ["Vetro", vr.vetro], ["Telaio", vr.telaio], ["Ala Z", vr.telaioAlaZ], ["Rifilato", vr.rifilato ? "Si" : "No"], ["Coprifilo", vr.coprifilo]], "sis" + idx)}
                  {Sez("COLORI", [["Interno", vr.coloreInt], ["Esterno", vr.coloreEst], ["Bicolore", vr.bicolore ? "Si" : "No"], ["Accessori", vr.coloreAcc]], "col" + idx)}
                  {Sez("MISURE", [["Larghezze", [mm.lAlto, mm.lCentro, mm.lBasso].filter(Boolean).join(" / ") || null], ["Altezze", [mm.hSx, mm.hCentro, mm.hDx].filter(Boolean).join(" / ") || null], ["Diagonali", [mm.d1, mm.d2].filter(Boolean).join(" / ") || null]], "mis" + idx)}
                  {Sez("ACCESSORI", [["Tapparella", ac.tapparella?.attivo ? "Si" : null], ["Persiana", ac.persiana?.attivo ? "Si" : null], ["Zanzariera", ac.zanzariera?.attivo ? "Si" : null]], "acc" + idx)}
                  {Sez("ALTRO", [["Difficolta", vr.difficoltaSalita], ["Mezzo", vr.mezzoSalita], ["Note", vr.note]], "alt" + idx)}
                </div>
              );
            })}
          </div>
