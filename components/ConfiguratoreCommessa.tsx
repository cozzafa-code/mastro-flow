...
export default function ConfiguratoreCommessa({ commessa, onClose }) {
  ...
  const [vani, setVani] = useState(
    commessa.vani || [{ id: Date.now(), nome: "Vetrina Negozio", misure: { lCentro: 2500, hCentro: 3000 }, disegno: {} }]
  );
  const [selIdx, setSelIdx] = useState(0);
  const vano = vani[selIdx];

  const onCadUpdate = (d) => {
    setVani(prev => prev.map((v, i) =>
      i === selIdx
        ? { ...v, disegno: d, misure: { lCentro: d.infisso?.larghezzaVano || d.L || v.misure.lCentro, hCentro: d.infisso?.altezzaVano || d.H || v.misure.hCentro } }
        : v
    ));
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "#1A1A1C", display: "flex", flexDirection: "column" }}>
      {/* TOPBAR */}
      <div style={{ height: 50, background: "#000", display: "flex", alignItems: "center", padding: "0 20px", justifyContent: "space-between", flexShrink: 0 }}>
      ...
      {/* CAD — occupa tutto lo spazio rimanente */}
