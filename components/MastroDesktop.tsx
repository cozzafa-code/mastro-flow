"use client";
// @ts-nocheck
// MASTRO — MastroDesktop v9 — VERSIONE FINALE
// 14 moduli attivi, routing completo, sidebar collassabile, badge live
...
const COMING_SOON:Record<string,[string,string,string]> = {
...
export default function MastroDesktop() {
  ...
  const daSdi = fattureDB.filter((f:any)=>!f.sdiInviata&&!f.pagata&&f.importo>0).length;

  const badge = (k:string) => ({
    messaggi:unread, commesse:ferme, contabilita:fatScad,
    fatture:fatScad+daSdi, montaggi:montaggiOggi
  }[k]||0);

  const currentLabel = NAV.flatMap(g=>g.items).find(n=>n.key===active)?.label||"Dashboard";

  const SoonView = ({k}:{k:string}) => {
    const p=COMING_SOON[k];
    if(!p) return <div style={{padding:60,textAlign:"center" as any,color:T.sub}}>Modulo in sviluppo</div>;
    return (
    ...
    <div style={{display:"flex",height:"100vh",width:"100vw",background:T.bg,fontFamily:FF,overflow:"hidden"}}>
      ...
      <div style={{width:sw,flexShrink:0,background:DARK,display:"flex",flexDirection:"column" as any,transition:"width .18s ease",overflow:"hidden",borderRight:"1px solid rgba(255,255,255,0.05)"}}>
        ...
        <div style={{height:52,display:"flex",alignItems:"center",padding:"0 12px",gap:10,borderBottom:"1px solid rgba(255,255,255,0.07)",flexShrink:0}}>
          <div style={{width:28,height:28,borderRadius:7,background:TEAL,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:900,color:"#fff",flexShrink:0,letterSpacing:-1}}>M</div>
              ...
              {items.map(({key,ico,label,sub,soon})=>{
                ...
                return (
                  <div key={key} onClick={()=>setTab(key)}
                    style={{display:"flex",alignItems:"center",gap:8,padding:collapsed?"7px 14px":"6px 14px",cursor:"pointer",position:"relative",background:on?"rgba(255,255,255,0.09)":"transparent",transition:"background .1s"}}
                    ...
                    {!collapsed&&<>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:12,fontWeight:on?500:400,color:on?"#fff":"rgba(255,255,255,0.45)",whiteSpace:"nowrap" as any,overflow:"hidden",textOverflow:"ellipsis"}}>{label}</div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column" as any,overflow:"hidden",minWidth:0}}>
        ...
        <div style={{height:50,flexShrink:0,background:"#fff",borderBottom:`0.5px solid ${T.bdr}`,display:"flex",alignItems:"center",padding:"0 18px",gap:10}}>
          ...
          <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:7}}>
            {ferme>0&&(
              <div onClick={()=>setTab("commesse")} style={{display:"flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:6,background:RED+"0E",border:`1px solid ${RED}20`,cursor:"pointer"}}>
                <div style={{width:4,height:4,borderRadius:"50%",background:RED}}/>
                <span style={{fontSize:11,fontWeight:500,color:RED}}>{ferme} ferme</span>
              </div>
            )}
            {montaggiOggi>0&&(
              <div onClick={()=>setTab("montaggi")} style={{display:"flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:6,background:PURPLE+"0E",border:`1px solid ${PURPLE}20`,cursor:"pointer"}}>
                <div style={{width:4,height:4,borderRadius:"50%",background:PURPLE}}/>
                <span style={{fontSize:11,fontWeight:500,color:PURPLE}}>{montaggiOggi} oggi</span>
              </div>
            )}
            {fatScad>0&&(
              <div onClick={()=>setTab("fatture")} style={{display:"flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:6,background:"#F9731610",border:"1px solid #F9731620",cursor:"pointer"}}>
                <div style={{width:4,height:4,borderRadius:"50%",background:"#F97316"}}/>
                <span style={{fontSize:11,fontWeight:500,color:"#F97316"}}>{fatScad} fatt. scadute</span>
              </div>
            )}
            {unread>0&&(
              <div onClick={()=>setTab("messaggi")} style={{display:"flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:6,background:"#3B7FE010",border:"1px solid #3B7FE020",cursor:"pointer"}}>
                <div style={{width:4,height:4,borderRadius:"50%",background:"#3B7FE0"}}/>
                <span style={{fontSize:11,fontWeight:500,color:"#3B7FE0"}}>{unread} msg</span>
              </div>
            )}
            {/* Avatar */}
            <div style={{width:28,height:28,borderRadius:"50%",background:TEAL+"16",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:TEAL,cursor:"pointer",marginLeft:4}} onClick={()=>setTab("settings")}>
