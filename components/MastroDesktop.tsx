"use client";
// @ts-nocheck
// MASTRO — MastroDesktop v13 — 22 MODULI
// Aggiunti: Listini, Trasporti (F5), CNC già dentro
...
const SoonView = ({ k, T }: any) => {
  const INFO: Record<string, [string, string, string]> = {
    trasporti: ["MASTRO TRASPORTI", "Rete logistica condivisa tra serramentisti. Matching furgoni/consegne per zona e data. Lancio mese 13.", "#F97316"],
  };
  const p = INFO[k] || ["In sviluppo", "Questo modulo sarà disponibile presto.", TEAL];
  return (
    <div style={{ display:"flex", flexDirection:"column" as any, alignItems:"center", justifyContent:"center", height:"70vh", gap:14, padding:40, textAlign:"center" as any }}>
      <div style={{ width:72, height:72, borderRadius:20, background:p[2]+"15", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke={p[2]} strokeWidth="1.5"><rect x="1" y="3" width="15" height="13" rx="1"/><path d="M16 8h4l3 4v4h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
      </div>
      ...
      <div style={{ padding:"8px 20px", borderRadius:8, background:p[2]+"10", border:`1px solid ${p[2]}25`, fontSize:12, fontWeight:500, color:p[2] }}>In roadmap</div>
    </div>
  );
};
