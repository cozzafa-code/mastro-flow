...
export default function MontaggiEditModalV2({ montaggio, commesse, montaggiTutti, aziendaId, onClose }: Props) {
  const [state, _setState] = useState<EditorState>(() => buildEditorState(montaggio));
  ...
  useEffect(() => {
    _setState(buildEditorState(montaggio));
    setErr(null);
  }, [montaggio]);
  ...
  const caricoMap = useMemo(() => {
    // Escludi il montaggio corrente dal carico (per non sommarlo a sé stesso)
  ...
  function handlePickCommessa(c: CommessaLite) {
    // Auto-fill contatto se commessa ha cliente
    setState({ commessaId: c.id });
    setShowCommessa(false);
  }
  function handlePickContatto(c: ContattoLite) {
    setState({ contattoId: c.id });
    if (state.tipo === "sopralluogo") {
      if (c.indirizzo && !state.indirizzoOverride) setState({ indirizzoOverride: c.indirizzo });
      if (c.telefono && !state.telefonoOverride) setState({ telefonoOverride: c.telefono });
    }
