        ...
        <div style={S.modalInner}>
          ...
          {showMailModal && (
            <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:500, display:"flex", alignItems:"flex-end", justifyContent:"center" }}
              ...
              <div style={{ background:L.bg, borderRadius:"20px 20px 0 0", width:"100%", maxWidth:480, maxHeight:"85vh", overflow:"auto", paddingBottom:24 }}>
                {/* Header */}
                <div style={{ padding:"16px 16px 10px", display:"flex", alignItems:"center", gap:10, position:"sticky", top:0, background:L.bg, zIndex:1, borderBottom:`1px solid ${T.bdr}` }}>
                  <span style={{ fontSize:22 }}>⚡</span>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:15, fontWeight:800, color:L.text }}>Manda Mail</div>
                  ...
                  <div onClick={() => setShowMailModal(null)} style={{ width:30, height:30, borderRadius:"50%", background:L.border, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:16, color:L.sub }}>ù</div>
                </div>

                <div style={{ padding:"14px 16px" }}>
                  {/* Info evento */}
                  <div style={{ background:L.amberBg, borderRadius:10, padding:"10px 12px", marginBottom:12, borderLeft:`3px solid ${T.acc}` }}>
                    <div style={{ fontSize:12, fontWeight:700, color:L.primary, marginBottom:2 }}>{showMailModal.ev.text}</div>
                    <div style={{ fontSize:11, color:L.sub }}>
                  ...
                  {!showMailModal.cm?.email && (
                    <div style={{ marginBottom:10 }}>
                      <div style={{ fontSize:10, fontWeight:700, color:L.sub, textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:4 }}>Email destinatario</div>
                      <input
                        type="email" placeholder="cliente@email.com"
                        style={{ width:"100%", padding:"9px 12px", borderRadius:8, border:`1px solid ${T.bdr}`, background:L.surface, fontSize:13, color:L.text, fontFamily:"inherit", boxSizing:"border-box" as any }}
                        onChange={e => {
                  ...
                  <div style={{ marginBottom:12 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:L.sub, textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:4 }}>Testo della mail</div>
                    <textarea
                      value={mailBody}
                      onChange={e => setMailBody(e.target.value)}
                      rows={10}
                      style={{ width:"100%", padding:"10px 12px", borderRadius:8, border:`1px solid ${T.bdr}`, background:L.surface, fontSize:12, color:L.text, fontFamily:"inherit", resize:"vertical" as any, boxSizing:"border-box" as any, lineHeight:1.6 }}
                    />
                  </div>

                  {/* Template rapidi */}
                  <div style={{ marginBottom:14 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:L.sub, textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:6 }}>Template rapidi</div>
...
Fabio Cozza - Walter Cozza Serramenti` },
                        ...
                        <div key={lbl} onClick={() => setMailBody(tpl)}
                          style={{ padding:"5px 10px", borderRadius:20, border:`1px solid ${T.bdr}`, background:L.surface, fontSize:11, fontWeight:600, color:L.text, cursor:"pointer" }}>
                          {lbl}
                        </div>
                      ))}
                    </div>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    <div
                      ...
                      style={{ flex:1, padding:"12px", borderRadius:10, background:L.primary, color:"#fff", textAlign:"center", cursor:"pointer", fontSize:13, fontWeight:700 }}>
                      ⚡ Apri in Mail
                    </div>
                    <div
                      onClick={() => {
                      ...
                      style={{ padding:"12px 14px", borderRadius:10, background:L.bg, border:`1px solid ${T.bdr}`, color:L.sub, cursor:"pointer", fontSize:13, fontWeight:600 }}>
                      📋 Copia
                    </div>
                  </div>
                </div>
          ...
          {showEmailComposer && (
            <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:500, display:"flex", alignItems:"flex-end", justifyContent:"center" }}
              ...
              <div style={{ background:L.surface, borderRadius:"20px 20px 0 0", width:"100%", maxWidth:500, maxHeight:"90vh", overflow:"auto", paddingBottom:24 }}>
                {/* Header */}
                <div style={{ padding:"16px 16px 10px", display:"flex", alignItems:"center", gap:10, position:"sticky", top:0, background:L.surface, zIndex:1, borderBottom:`1px solid ${T.bdr}` }}>
                  <span style={{ fontSize:22 }}>⚡</span>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:15, fontWeight:800, color:L.text }}>Componi Email</div>
                    <div style={{ fontSize:11, color:L.sub }}>{showEmailComposer.cm?.code} · {showEmailComposer.cm?.cliente} {showEmailComposer.cm?.cognome||""}</div>
                  </div>
                  <div onClick={() => setShowEmailComposer(null)} style={{ width:30, height:30, borderRadius:"50%", background:L.border, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:16, color:L.sub }}>ù</div>
                </div>
                <div style={{ padding:"14px 16px" }}>
                  {/* Template selector */}
                  ...
                  {/* Destinatario */}
                  <div style={{ marginBottom:10 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:L.sub, textTransform:"uppercase", marginBottom:4 }}>A:</div>
                    <input value={emailDest} onChange={e => setEmailDest(e.target.value)} placeholder="email@cliente.it"
                      style={{ width:"100%", padding:"9px 12px", borderRadius:8, border:`1px solid ${T.bdr}`, background:L.bg, fontSize:13, color:L.text, fontFamily:"inherit", boxSizing:"border-box" as any }} />
                  </div>
                  {/* Oggetto */}
                  <div style={{ marginBottom:10 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:L.sub, textTransform:"uppercase", marginBottom:4 }}>Oggetto:</div>
                    <input value={emailOggetto} onChange={e => setEmailOggetto(e.target.value)}
                      style={{ width:"100%", padding:"9px 12px", borderRadius:8, border:`1px solid ${T.bdr}`, background:L.bg, fontSize:13, color:L.text, fontFamily:"inherit", boxSizing:"border-box" as any }} />
                  </div>
                  {/* Corpo */}
                  <div style={{ marginBottom:14 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:L.sub, textTransform:"uppercase", marginBottom:4 }}>Messaggio:</div>
                    <textarea value={emailCorpo} onChange={e => setEmailCorpo(e.target.value)} rows={14}
                      style={{ width:"100%", padding:"10px 12px", borderRadius:8, border:`1px solid ${T.bdr}`, background:L.bg, fontSize:12, color:L.text, fontFamily:"inherit", resize:"vertical" as any, boxSizing:"border-box" as any, lineHeight:1.6 }} />
                  </div>
                  ...
                  <div onClick={() => { navigator.clipboard?.writeText(emailCorpo); alert("Copiato!"); }}
                    style={{ marginTop:8, padding:10, borderRadius:8, background:L.bg, border:`1px solid ${T.bdr}`, textAlign:"center", cursor:"pointer", fontSize:12, fontWeight:600, color:L.sub }}>
                    📋 Copia testo
                  </div>
                </div>
              </div>
            ...
            <>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {[{ id: "alta", l: "Urgente", c: L.red }, { id: "media", l: "Normale", c: L.amber }, { id: "bassa", l: "Bassa", c: L.sub }].map(p => (
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {[
                    ...
                    <div key={i} onClick={b.act} style={{ flex: 1, padding: "8px 4px", background: L.bg, borderRadius: 8, border: `1px solid ${T.bdr}`, textAlign: "center", cursor: "pointer" }}>
                      <div style={{ fontSize: 16 }}>{b.ico}</div>
                      <div style={{ fontSize: 9, fontWeight: 600, color: L.sub, marginTop: 1 }}>{b.l}</div>
                    </div>
                  ))}
                </div>
                {taskAllegati.length > 0 && (
                  <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {taskAllegati.map(a => (
                      <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 4, padding: "3px 8px", borderRadius: 6, background: L.bg, border: `1px solid ${T.bdr}`, fontSize: 10 }}>
                        <span>{a.tipo === "nota" ? "📝" : a.tipo === "vocale" ? "🎵" : a.tipo === "foto" ? "📸" : "📁"}</span>
                        <span style={{ color: L.text, maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.nome}</span>
                        <span onClick={() => setTaskAllegati(al => al.filter(x => x.id !== a.id))} style={{ cursor: "pointer", color: L.red }}>×</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {showModal === "commessa" && (
            <>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
                {[{ id: "nuova", l: "Nuova installazione", c: L.primary }, { id: "riparazione", l: "Riparazione", c: L.amber }].map(t => (
                  <div key={t.id} onClick={() => { setNewCM(c => ({ ...c, tipo: t.id })); setRipSearch(""); setRipCMSel(null); setRipProblema(""); setRipFotos([]); setRipUrgenza("media"); }}
                    style={{ flex: 1, padding: "12px 6px", borderRadius: 12, border: `2px solid ${newCM.tipo === t.id ? (t.id==="nuova"?T.acc:T.orange) : T.bdr}`, background: newCM.tipo === t.id ? (t.id==="nuova"?L.primary:L.amber)+"12" : L.surface, textAlign: "center", cursor: "pointer", transition:"all 0.15s" }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: newCM.tipo === t.id ? (t.id==="nuova"?L.primary:L.amber) : L.sub }}>{t.l}</div>
                  </div>
                ))}
              </div>
              ...
              {newCM.tipo === "riparazione" && (() => {
                ...
                const colorUrgenza = ripUrgenza === "urgente" ? L.red : ripUrgenza === "normale" ? L.green : L.amber;
                return (
                  <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
                        ...
                        <React.Fragment key={s}>
                          <div onClick={() => i < ripStep && setRipStep(i)}
                            ...
                            <div style={{ width:28, height:28, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:700,
                              background: i < ripStep ? L.green : i === ripStep ? L.amber : L.border,
                              color: i <= ripStep ? "#fff" : L.sub }}>
                              {i < ripStep ? "✓" : i+1}
                            </div>
                            <div style={{ fontSize:9, fontWeight: i===ripStep?700:500, color: i===ripStep?L.amber:L.sub, marginTop:3 }}>{s}</div>
                          </div>
                          {i < STEPS_RIP.length-1 && <div style={{ flex:1, height:2, background: i < ripStep ? L.green : L.border, marginBottom:14 }} />}
                        </React.Fragment>
                      ))}
                    </div>
                    ...
                    {ripStep === 0 && (
                      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                        ...
                        {ripSearch.length >= 1 && !ripCMSel && (() => {
                          ...
                          return (
                            <div style={{ background:L.surface, border:`1px solid ${T.bdr}`, borderRadius:10, overflow:"hidden" }}>
                              {ctR.map(ct => (
                                <div key={ct.id} onClick={() => { setRipSearch(ct.nome+" "+(ct.cognome||"")); setNewCM(x=>({...x,cliente:ct.nome,cognome:ct.cognome||"",indirizzo:ct.indirizzo||"",telefono:ct.telefono||""})); }}
                                  ...
                                  <div style={{ width:28,height:28,borderRadius:"50%",background:L.amberBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:L.primary }}>
                                    {(ct.nome||"?")[0]}
                                  </div>
                                  <div><div style={{ fontSize:13,fontWeight:700 }}>{ct.nome} {ct.cognome||""}</div><div style={{ fontSize:10,color:L.sub }}>{ct.telefono}</div></div>
                                </div>
                              ))}
                              {cmR.map(c => (
                                <div key={c.id} onClick={() => { setRipCMSel(c); setRipSearch(c.cliente); setNewCM(x=>({...x,indirizzo:c.indirizzo,telefono:c.telefono})); }}
                                  ...
                                  <div><div style={{ fontSize:13,fontWeight:700 }}>{c.cliente}</div><div style={{ fontSize:10,color:L.sub }}>{c.code} · {c.indirizzo}</div></div>
                                  <div style={{ fontSize:11,color:L.primary,fontWeight:700 }}>Collega →</div>
                                </div>
                              ))}
                            </div>
                          );
                        ...
                        {ripCMSel && (
                          <div style={{ padding:"10px 12px", background:L.amberBg, border:`1px solid ${T.acc}30`, borderRadius:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                            <div><div style={{ fontSize:12,fontWeight:700,color:L.primary }}>✓ {ripCMSel.code}</div><div style={{ fontSize:11,color:L.sub }}>{ripCMSel.cliente}</div></div>
                            <div onClick={() => { setRipCMSel(null); setRipSearch(""); }} style={{ color:L.sub,cursor:"pointer",fontSize:18 }}>×</div>
                          </div>
                        )}
                        {!ripCMSel && (
                          <div style={{ display:"flex", flexDirection:"column", gap:8, padding:"10px 12px", background:L.bg, borderRadius:10, border:`1px solid ${T.bdr}` }}>
                            <div style={{ fontSize:10,fontWeight:700,color:L.sub,textTransform:"uppercase",letterSpacing:"0.06em" }}>Cliente nuovo</div>
                        ...
                        <div style={{ display:"flex", gap:8, marginTop:4 }}>
                          <button style={{ ...S.btn, background:L.surface, color:L.sub, border:`1px solid ${T.bdr}`, flex:1 }}
                            onClick={() => { setNewCM(c=>({...c,tipo:"nuova"})); setRipSearch(""); setRipCMSel(null); setRipStep(0); setShowModal(null); }}>
                            Annulla
                          </button>
                          <button style={{ ...S.btn, background: canNext0 ? L.amber : "#ccc", cursor: canNext0 ? "pointer" : "not-allowed", flex:2 }}
                    ...
                    {ripStep === 1 && (
                      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                        <div>
                          ...
                          <div style={{ display:"flex", gap:8 }}>
                            {[{id:"normale",l:"Normale",c:L.green},{id:"media",l:"Media",c:L.amber},{id:"urgente",l:"Urgente",c:L.red}].map(u => (
                              <div key={u.id} onClick={() => setRipUrgenza(u.id)}
                                style={{ flex:1, padding:"12px 4px", borderRadius:12, border:`2px solid ${ripUrgenza===u.id?u.c:T.bdr}`, background:ripUrgenza===u.id?u.c+"18":L.surface, textAlign:"center", cursor:"pointer",
                                  boxShadow: ripUrgenza===u.id ? `0 3px 0 ${u.c}40` : "0 2px 0 rgba(0,0,0,0.08)", transition:"all 0.1s" }}>
                                <div style={{ fontSize:13,fontWeight:700,color:ripUrgenza===u.id?u.c:L.sub }}>{u.l}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                          ...
                          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                            {["Vetro rotto","Cardine","Guarnizione","Serratura","Maniglia","Tapparella","Infiltrazioni","Deformazione","Altro"].map(t => (
                              <div key={t} onClick={() => setNewCM(c=>({...c,tipoProblema:c.tipoProblema===t?"":t}))}
                                style={{ padding:"7px 12px", borderRadius:20, border:`1.5px solid ${newCM.tipoProblema===t?T.orange:T.bdr}`, background:newCM.tipoProblema===t?L.amber+"18":L.surface, fontSize:12, fontWeight:600, color:newCM.tipoProblema===t?L.amber:L.sub, cursor:"pointer",
                          ...
                          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                            {["Finestra","Porta","Portafinestra","Scorrevole","Tapparella","Persiana","Zanzariera","Altro"].map(t => (
                              <div key={t} onClick={() => setNewCM(c=>({...c,tipoInfisso:c.tipoInfisso===t?"":t}))}
                                style={{ padding:"7px 12px", borderRadius:20, border:`1.5px solid ${newCM.tipoInfisso===t?T.acc:T.bdr}`, background:newCM.tipoInfisso===t?L.amberBg:L.surface, fontSize:12, fontWeight:600, color:newCM.tipoInfisso===t?L.primary:L.sub, cursor:"pointer",
                        ...
                        <div style={{ display:"flex", gap:8 }}>
                          <button style={{ ...S.btn, background:L.surface, color:L.sub, border:`1px solid ${T.bdr}`, flex:1 }} onClick={() => setRipStep(0)}>← Indietro</button>
                          <button style={{ ...S.btn, background:canNext1?L.amber:"#ccc", cursor:canNext1?"pointer":"not-allowed", flex:2 }} onClick={() => canNext1 && setRipStep(2)}>Avanti →</button>
                        </div>
                      </div>
                    )}
                    ...
                    {ripStep === 2 && (
                      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                          ...
                          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                            {["Cliente","Posatore","Tecnico","Subappaltatore","Altro"].map(t => (
                              <div key={t} onClick={() => setNewCM(c=>({...c,chiSegnala:c.chiSegnala===t?"":t}))}
                                style={{ padding:"7px 12px", borderRadius:20, border:`1.5px solid ${newCM.chiSegnala===t?T.acc:T.bdr}`, background:newCM.chiSegnala===t?L.amberBg:L.surface, fontSize:12, fontWeight:600, color:newCM.chiSegnala===t?L.primary:L.sub, cursor:"pointer",
                        ...
                        <div style={{ display:"flex", gap:8 }}>
                          <button style={{ ...S.btn, background:L.surface, color:L.sub, border:`1px solid ${T.bdr}`, flex:1 }} onClick={() => setRipStep(1)}>← Indietro</button>
                          <button style={{ ...S.btn, background:L.amber, flex:2 }} onClick={() => setRipStep(3)}>Avanti →</button>
                        </div>
                      </div>
                    )}
                        ...
                        <div>
                          ...
                          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                            ...
                            <div onClick={() => ripFotoRef.current?.click()}
                              ...
                              <div style={{ fontSize:24, color:L.sub }}>+</div>
                              <div style={{ fontSize:9, color:L.sub, fontWeight:600 }}>Foto</div>
                            </div>
                          </div>
                          <input ref={ripFotoRef} type="file" accept="image/*" capture="environment" style={{ display:"none" }} onChange={addRipFoto} />
                        </div>
                        {/* Riepilogo */}
                        <div style={{ padding:"12px 14px", background:L.amber+"10", borderRadius:12, border:`1px solid ${T.orange}30` }}>
                          <div style={{ fontSize:11, fontWeight:700, color:L.amber, marginBottom:6 }}>Riepilogo riparazione</div>
                          <div style={{ fontSize:12, color:L.text }}>{ripCMSel ? ripCMSel.cliente : newCM.cliente || ripSearch}</div>
                          <div style={{ fontSize:11, color:L.sub, marginTop:2 }}>{newCM.tipoProblema && newCM.tipoProblema+" · "}{newCM.tipoInfisso && newCM.tipoInfisso+" · "}<span style={{ color:colorUrgenza, fontWeight:700 }}>{labelUrgenza}</span></div>
                          <div style={{ fontSize:11, color:L.sub, marginTop:2, fontStyle:"italic" }}>{ripProblema.slice(0,60)}{ripProblema.length>60?"...":""}</div>
                        </div>
                        <div style={{ display:"flex", gap:8 }}>
                          <button style={{ ...S.btn, background:L.surface, color:L.sub, border:`1px solid ${T.bdr}`, flex:1 }} onClick={() => setRipStep(2)}>← Indietro</button>
                          <button style={{ ...S.btn, background:L.amber, flex:2 }} onClick={addRiparazione}>Crea riparazione ✓</button>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {newCM.tipo === "nuova" && (() => {
                  ...
                  return (
                    <div style={{ marginBottom:8, borderRadius:10, border:`1px solid ${T.bdr}`, overflow:"hidden" }}>
                      <div onClick={() => setNewCM(c=>({...c,_open:open?null:id}))}
                        style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"11px 14px", background:L.surface, cursor:"pointer" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <span style={{ fontSize:16 }}>{icon}</span>
                          <span style={{ fontSize:13, fontWeight:600, color:L.text }}>{label}</span>
                          {badge && <span style={{ ...S.badge(L.amberBg,L.primary), fontSize:10 }}>{badge}</span>}
                        </div>
                        <span style={{ fontSize:12, color:L.sub, transition:"transform 0.2s", display:"inline-block", transform:open?"rotate(180deg)":"rotate(0deg)" }}>>▾</span>
                      </div>
                      {open && <div style={{ padding:"12px 14px", background:L.bg, borderTop:`1px solid ${T.bdr}` }}>{children}</div>}
                    </div>
                  );
                };
                ...
                return (
                  <>
                    <div style={{ marginBottom:14, padding:"8px 12px", background:L.bg, borderRadius:8, display:"flex", alignItems:"center", gap:8 }}>
                      <span style={{ fontSize:11, color:L.sub }}>Numero commessa:</span>
                      <span style={{ fontSize:13, fontWeight:800, color:L.primary, fontFamily:FM }}>{previewCode}</span>
                      <span style={{ fontSize:10, color:L.sub }}>(assegnato automaticamente)</span>
                    </div>

                    <div style={{ marginBottom:14, padding:"14px", background:L.surface, borderRadius:12, border:`1.5px solid ${T.bdr}` }}>
                      <div style={{ fontSize:10, fontWeight:800, color:L.sub, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:10 }}>👤 Dati cliente *</div>
                      ...
                      {!newCM.cliente && (
                        <div onClick={() => setNewCM(c => ({...c, cliente: " "}))} style={{ margin:"6px 0 8px", padding:"10px 14px", borderRadius:10, border:"1.5px dashed "+L.primary+"60", background:L.primary+"06", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                          <span style={{ fontSize:16 }}>📊</span>
                          <span style={{ fontSize:12, fontWeight:700, color:L.primary }}>Scegli dalla rubrica ({contatti.filter(c=>c.tipo==="cliente").length} clienti)</span>
                        </div>
                      )}
                      ...
                      {newCM.cliente.length >= 1 && (() => {
                        ...
                        return <div style={{ background:"#fff", border:"1.5px solid " + L.primary + "40", borderRadius:10, marginTop:6, marginBottom:8, overflow:"hidden", maxHeight:220, overflowY:"auto" }}>
                          <div style={{ padding:"6px 10px", fontSize:9, fontWeight:700, color:L.primary, background:L.primary+"08", borderBottom:"1px solid " + L.primary + "20", display:"flex", justifyContent:"space-between", position:"sticky", top:0 }}>
                            <span>📊 Seleziona dalla rubrica ({unique.length})</span>
                            <span onClick={(e) => { e.stopPropagation(); setNewCM(c => ({...c, cliente: ""})); }} style={{ cursor:"pointer", color:L.sub }}>×</span>
                          </div>
                          {unique.map((s, i) => (
                            <div key={i} onClick={() => setNewCM(c => ({...c, cliente: s.nome, cognome: s.cognome, telefono: s.tel, email: s.email, indirizzo: s.indirizzo || c.indirizzo }))}
                              style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 12px", cursor:"pointer", borderBottom: i < unique.length -1 ? "1px solid " + L.border + "40" : "none" }}>
                              <div style={{ fontSize:16 }}>{s.ico}</div>
                              <div style={{ flex:1 }}>
                                <div style={{ fontSize:12, fontWeight:700, color:L.text }}>{s.nome} {s.cognome}</div>
                                <div style={{ fontSize:9, color:L.sub }}>{s.tel && ("📞 " + s.tel)} {s.email && (" ⚡ " + s.email)}</div>
                                {s.indirizzo && <div style={{ fontSize:9, color:L.sub }}>📧 {s.indirizzo}</div>}
                              </div>
                              <span style={{ fontSize:8, padding:"2px 6px", borderRadius:4, background: s.tipo === "rubrica" ? "#af52de15" : L.amberBg, color: s.tipo === "rubrica" ? "#af52de" : L.primary, fontWeight:700 }}>{s.tipo === "rubrica" ? "RUBRICA" : "COMMESSA"}</span>
                            </div>
                          ))}
                        </div>;
                      })()}
                      ...
                      <div style={{ display:"flex", gap:4, marginBottom:8 }}>
                        {[{id:"facile",l:"Facile",c:L.green,e:"📋"},{id:"media",l:"Media",c:L.amber,e:"🚨"},{id:"difficile",l:"Difficile",c:L.red,e:""}].map(d => (
                          <div key={d.id} onClick={()=>setNewCM(c=>({...c,difficoltaSalita:d.id}))}
                            style={{ flex:1, padding:"8px 4px", borderRadius:8, border:`1.5px solid ${newCM.difficoltaSalita===d.id?d.c:T.bdr}`, background:newCM.difficoltaSalita===d.id?d.c+"15":L.surface, textAlign:"center", cursor:"pointer" }}>
                            <div style={{ fontSize:14 }}>{d.e}</div>
                            <div style={{ fontSize:10, fontWeight:600, color:newCM.difficoltaSalita===d.id?d.c:L.sub }}>{d.l}</div>
                          </div>
                        ))}
                      </div>
                      <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:10, color:L.sub, fontWeight:600, marginBottom:2 }}>Piano edificio</div>
                        ...
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:10, color:L.sub, fontWeight:600, marginBottom:2 }}>Foro scale (cm)</div>
                          <input style={S.input} placeholder="es. 80ù200" value={newCM.foroScale} onChange={e=>setNewCM(c=>({...c,foroScale:e.target.value}))}/>
                        </div>
                      </div>
                      <div style={{ fontSize:10, color:L.sub, fontWeight:600, marginBottom:2 }}>Mezzo di salita</div>
                    ...
                    <div style={{ marginTop:6 }}>
                      {!newCM.cliente.trim() && (
                        <div style={{ fontSize:11, color:L.sub, textAlign:"center", marginBottom:8 }}>Inserisci almeno il nome per procedere</div>
                      )}
                      <button style={{ ...S.btn, background:newCM.cliente.trim()?L.primary:"#ccc", cursor:newCM.cliente.trim()?"pointer":"not-allowed" }}
