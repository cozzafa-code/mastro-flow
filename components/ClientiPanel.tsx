...
const tipoEvColor = (tipo) => {
  const map = { sopralluogo: "#3B7FE0", montaggio: "#E8A020", consegna: "#0D7C6B", preventivo: L.amber, posa: "#059669", collaudo: "#10B981", chiamata: "#af52de" };
  return map[tipo] || L.amber;
};
...
const DIARIO_TAGS = [
  ...
  { id: "problema", label: "Problema", color: L.red },
];
          ...
          <div style={{ padding: "0 16px" }}>
            {[
              ...
              <div key={f.k} style={{ marginBottom: 10 }}>
                ...
                <input value={editForm[f.k] || ""} onChange={e => setEditForm(prev => ({...prev, [f.k]: e.target.value}))}
                  placeholder={f.ph} style={{ width: "100%", padding: "10px 12px", borderRadius: 12, border: `1px solid ${L.border}`, background: L.surface, fontSize: 13, color: L.text, fontFamily: "inherit", boxSizing: "border-box" }} />
              </div>
            ))}
            <div style={{ marginBottom: 10 }}>
              ...
              <textarea value={editForm.note || ""} onChange={e => setEditForm(prev => ({...prev, note: e.target.value}))}
                placeholder="Note..." rows={3} style={{ width: "100%", padding: "10px 12px", borderRadius: 12, border: `1px solid ${L.border}`, background: L.surface, fontSize: 13, color: L.text, fontFamily: "inherit", boxSizing: "border-box", resize: "vertical" }} />
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
              <button onClick={() => { setEditMode(false); setEditForm(null); }} style={{ flex: 1, padding: 14, borderRadius: 16, border: `1px solid ${L.border}`, background: L.bg, fontSize: 14, fontWeight: 600, cursor: "pointer", color: L.text, fontFamily: "inherit" }}>Annulla</button>
              <button onClick={() => {
              ...
              }} style={{ flex: 2, padding: 14, borderRadius: 16, border: "none", background: L.primary, fontSize: 14, fontWeight: 700, cursor: "pointer", color: "#fff", fontFamily: "inherit" }}>Salva</button>
            </div>
          </div>
        </div>
      );
        ...
        <div style={{ padding: "0 0 100px" }}>
          ...
          <div style={{ ...S.header, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div onClick={() => { setEditForm({ ...c }); setEditMode(true); }}
              style={{ width: 34, height: 34, borderRadius: 12, background: L.bg, border: "1px solid " + L.border, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, margin: "0 16px 12px" }}>
            <div style={{ background: L.surface, borderRadius: 16, padding: "12px 10px", textAlign: "center", border: "1px solid " + L.border }}>
            ...
            <div style={{ background: L.surface, borderRadius: 16, padding: "12px 10px", textAlign: "center", border: "1px solid " + L.border }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: #e4c18c }}>{fattureTot > 0 ? (fattureTot / 1000).toFixed(1) + "k" : "0"}</div>
              <div style={{ fontSize: 10, color: L.sub, fontWeight: 600 }}>€ Totale</div>
            </div>
          </div>

          {/* Tab bar */}
          <div style={{ display: "flex", gap: 0, margin: "0 16px 12px", background: L.bg, borderRadius: 14, padding: 3 }}>
            {tabs.map(t => (
              <div key={t.id} onClick={() => setClienteDetailTab(t.id)}
                style={{ flex: 1, padding: "8px 4px", textAlign: "center", borderRadius: 12, fontSize: 11, fontWeight: 700,
          ...
          {clienteDetailTab === "info" && <>
            <div style={{ margin: "0 16px 12px", background: L.surface, borderRadius: 18, padding: 16, border: "1px solid " + L.border }}>
              {c.telefono && <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                <div onClick={() => { window.location.href="tel:" + c.telefono; }} style={{ padding: "6px 12px", borderRadius: 12, background: #d1fae5, color: L.green, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Chiama</div>
                <div onClick={() => window.open("https://wa.me/" + c.telefono.replace(/\s/g, ""))} style={{ padding: "6px 12px", borderRadius: 12, background: "#dcf8c6", color: "#128c7e", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>WA</div>
              </div>}
              {c.email && <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                <div onClick={() => window.open("mailto:" + c.email)} style={{ padding: "6px 12px", borderRadius: 12, background: #dbeafe, color: #3b7fe0, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Email</div>
              </div>}
              {c.indirizzo && <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                <div onClick={() => window.open("https://maps.google.com/?q=" + encodeURIComponent(c.indirizzo))} style={{ padding: "6px 12px", borderRadius: 12, background: L.bg, color: L.sub, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Mappa</div>
              </div>}
              ...
              {c.note && <div style={{ marginTop: 10, padding: "10px 12px", borderRadius: 14, background: L.primary + "08", border: "1px solid " + L.primary + "15" }}>
            ...
            <div style={{ margin: "0 16px 12px" }}>
              ...
              {cmList.length === 0 && <div style={{ padding: 16, background: L.surface, borderRadius: 14, textAlign: "center", fontSize: 12, color: L.sub }}>Nessuna commessa</div>}
              {cmList.map(cm => (
                <div key={cm.id} onClick={() => { setSelectedCM(cm); setTab("commesse"); }} style={{ padding: "10px 12px", background: L.surface, borderRadius: 14, border: "1px solid " + L.border, marginBottom: 6, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 12, background: (PIPELINE.find(p => p.id === cm.fase)?.color || L.primary) + "18", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>
          ...
          {clienteDetailTab === "diario" && <>
            <div style={{ margin: "0 16px 12px" }}>
              {/* New entry */}
              <div style={{ background: L.surface, borderRadius: 18, padding: 14, border: "1px solid " + L.border, marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 8, flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: 1 }} />
                  <div onClick={addDiarioEntry} style={{ padding: "5px 14px", background: L.primary, color: "#fff", borderRadius: 12, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Salva</div>
                </div>
              </div>
              ...
              {diario.map(d => {
                ...
                return (
                  <div key={d.id} style={{ background: L.surface, borderRadius: 16, padding: "12px 14px", marginBottom: 8, border: "1px solid " + L.border, borderLeft: "3px solid " + tagObj.color }}>
          ...
          {clienteDetailTab === "storia" && <>
            <div style={{ margin: "0 16px 12px" }}>
              ...
              {[...evList].sort((a, b) => (b.date || "").localeCompare(a.date || "")).length === 0 &&
                <div style={{ padding: 24, background: L.surface, borderRadius: 16, textAlign: "center", fontSize: 12, color: L.sub }}>Nessuna attività registrata</div>
              }
              ...
              {(() => {
                ...
                return <>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
                    <div style={{ background: L.surface, borderRadius: 16, padding: 14, border: "1px solid " + L.border }}>
                  ...
                  {fattList.length === 0 && <div style={{ padding: 24, background: L.surface, borderRadius: 16, textAlign: "center", fontSize: 12, color: L.sub }}>Nessuna fattura</div>}
                  {fattList.map(f => (
                    <div key={f.id} style={{ padding: "10px 12px", background: L.surface, borderRadius: 14, border: "1px solid " + L.border, marginBottom: 6, display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div style={{ fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 8, background: f.pagata ? #d1fae5 : #ffdad6, color: f.pagata ? L.green : L.red }}>{f.pagata ? "Pagata" : "Da pagare"}</div>
                    </div>
                  ))}
                </>;
              })()}
          ...
          <div style={{ margin: "12px 16px 0", display: "flex", gap: 8 }}>
            <div onClick={() => { if(confirm("Eliminare " + c.nome + "?")) { setContatti(prev => prev.filter(x => x.id !== c.id)); setSelectedCliente(null); }}} style={{ flex: 1, padding: 12, borderRadius: 14, background: #ffdad6, color: L.red, textAlign: "center", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Elimina</div>
          </div>
        </div>
      );
    }
          ...
          <div style={{ padding: "0 16px" }}>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <textarea value={newCliente.note} onChange={e => setNewCliente(prev => ({...prev, note: e.target.value}))}
                placeholder="Note aggiuntive..." rows={3} style={{ width: "100%", padding: "10px 12px", borderRadius: 12, border: `1px solid ${L.border}`, background: L.surface, fontSize: 13, color: L.text, fontFamily: "inherit", boxSizing: "border-box", resize: "vertical" }} />
            </div>
            {savedCliente ? (
              <div style={{ marginTop: 8 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <div onClick={() => { setNewCM(prev => ({ ...prev, cliente: savedCliente.nome, cognome: savedCliente.cognome || "", telefono: savedCliente.telefono || "", indirizzo: savedCliente.indirizzo || "" })); setTab("commesse"); setShowNewCliente(false); setSavedCliente(null); }} style={{ padding: "14px 16px", borderRadius: 16, background: L.primary, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
                    <I d={ICO.folder} s={18} c="#fff" /> Crea commessa per {savedCliente.nome}
                  </div>
                  <div onClick={() => { setTab("agenda"); setShowNewCliente(false); setSavedCliente(null); }} style={{ padding: "14px 16px", borderRadius: 16, background: #3b7fe0, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
                    <I d={ICO.check} s={18} c="#fff" /> Aggiungi task / appuntamento
                  </div>
                  <div onClick={() => { setTab("agenda"); setShowNewCliente(false); setSavedCliente(null); }} style={{ padding: "14px 16px", borderRadius: 16, background: "#5856d6", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
                    <I d={ICO.calendar} s={18} c="#fff" /> Crea appuntamento
                  </div>
                  <div onClick={() => { setShowNewCliente(false); setSavedCliente(null); }} style={{ padding: "14px 16px", borderRadius: 16, background: L.bg, color: L.sub, fontSize: 14, fontWeight: 600, cursor: "pointer", textAlign: "center", border: `1px solid ${L.border}` }}>
                    Chiudi
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                }} style={{ width: "100%", padding: "14px", borderRadius: 16, border: "none", background: L.green, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                ...
                <button onClick={() => {
                  ...
                  setNewCliente({ nome: "", cognome: "", tipo: "cliente", telefono: "", email: "", indirizzo: "", piva: "", note: "" });
                }} style={{ width: "100%", padding: "14px", borderRadius: 16, border: "none", background: L.amber, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      <div style={{ padding: "0 0 100px" }}>
        <div style={{ ...S.header, flexDirection: "column", alignItems: "stretch" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div onClick={() => setShowNewCliente(true)} style={{ width: 32, height: 32, borderRadius: 12, background: L.primary, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                <Ico d={ICO.plus} s={16} c="#fff" />
              </div>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 14, border: `1px solid ${L.border}`, background: L.surface, marginBottom: 8 }}>
          ...
          <div style={{ display: "flex", gap: 4, overflowX: "auto", WebkitOverflowScrolling: "touch", paddingBottom: 2 }}>
            {filters.map(f => (
              <div key={f.id} onClick={() => setClientiFilter(f.id)} style={{ padding: "5px 10px", borderRadius: 20, border: `1px solid ${clientiFilter === f.id ? f.c : L.border}`, background: clientiFilter === f.id ? f.c + "15" : L.surface, fontSize: 10, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap", color: clientiFilter === f.id ? f.c : L.sub }}>
        ...
        <div style={{ padding: "0 16px" }}>
          ...
          {filtered.map(c => {
            ...
            return (
              <div key={c.id} onClick={() => setSelectedCliente(c)} style={{ padding: "12px 14px", background: L.surface, borderRadius: 16, border: `1px solid ${L.border}`, marginBottom: 6, cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }}>
