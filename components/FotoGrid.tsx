"use client";
// components/FotoGrid.tsx - Griglia foto professionale con icone SVG

import React, { useState, useRef } from "react";
import { uploadMultipleFiles } from "../lib/uploadStorage";
import { IcoCamera, IcoClose, IcoRefresh } from "./IconLib";
...
export default function FotoGrid({ foto, onChange, uploadFolder = 'notes', readOnly = false, size = 'md', maxFoto = 10, label }: Props) {
  ...
  async function handleFiles(files: FileList | null) {
    ...
    if (foto.length + files.length > maxFoto) { alert(`Max ${maxFoto} foto`); return; }
    setUploading(true);
    const urls = await uploadMultipleFiles(uploadFolder, files);
    if (urls.length > 0) onChange([...foto, ...urls]);
    ...
    <>
        ...
        {foto.map((url, i) => (
          <div key={i} style={{ position: 'relative' as const, width: tileSize, height: tileSize }}>
            <img src={url} alt="" onClick={() => setLightbox(url)} style={{
              ...
              borderRadius: 8, cursor: 'pointer', border: '1.5px solid #E5EAF0',
            }} />
            {!readOnly && (
        ...
        {!readOnly && foto.length < maxFoto && (
          <>
            <button onClick={() => inputRef.current?.click()} disabled={uploading} style={{
              ...
              background: uploading ? '#F1F4F7' : '#F8FAFA',
              ...
              {uploading ? (
                <>
                  <IcoRefresh size={20} color={MUTED} />
                  <span style={{ fontSize: 9, fontWeight: 700, color: MUTED }}>Upload...</span>
                </>
              ) : (
                <>
                  <IcoCamera size={22} color={TEAL_DEEP} />
                  <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: 0.3 }}>AGGIUNGI</span>
      ...
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{
          position: 'fixed', inset: 0, background: 'rgba(15,27,45,0.95)', zIndex: 10100,
