...
export default function AgendaFiltersMobile({ open, current, onClose, onApply }: Props) {
  ...
  const Toggle = ({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) => (
    <div onClick={() => onChange(!value)} style={{ width: 44, height: 24, borderRadius: 12, background: value ? "#28A0A0" : "#D4D4D8", position: "relative", cursor: "pointer", transition: "background 0.2s", flexShrink: 0 }}>
  ...
  const ToggleRow = ({ label, dotColor, value, onChange, sub }: { label: string; dotColor: string; value: boolean; onChange: (v: boolean) => void; sub?: string }) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 10, height: 10, borderRadius: "50%", background: dotColor, flexShrink: 0 }} />
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: "#0D1F1F" }}>{label}</div>
  ...
  const SectionTitle = ({ children }: { children: React.ReactNode }) => (
    <div style={{ fontSize: 11, fontWeight: 900, color: "#28A0A0", letterSpacing: 1, marginTop: 22, marginBottom: 4 }}>{children}</div>
  );
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#fff", zIndex: 1200, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "18px" }}>
        <div style={{ fontSize: 11, fontWeight: 900, color: "#28A0A0", letterSpacing: 1, marginBottom: 4 }}>OPERATIVO</div>
        ...
        <ToggleRow label="Preventivi in attesa"  sub="Inviati ma senza risposta"        dotColor={TIPO_COLORS.preventivo.bd} value={draft.showPreventivi} onChange={(v) => setDraft({ ...draft, showPreventivi: v })} />
        <ToggleRow label="Da firmare"             sub="Conferme accettate ma non firmate" dotColor={TIPO_COLORS.firma.bd}      value={draft.showFirme}      onChange={(v) => setDraft({ ...draft, showFirme: v })} />
        <ToggleRow label="Acconto da fatturare"   sub="Firmati senza fattura acconto"     dotColor={TIPO_COLORS.acconto.bd}    value={draft.showAcconti}    onChange={(v) => setDraft({ ...draft, showAcconti: v })} />
        <ToggleRow label="Saldi in scadenza"      sub="Fatture saldo non pagate (30gg)"  dotColor={TIPO_COLORS.saldo.bd}      value={draft.showSaldi}      onChange={(v) => setDraft({ ...draft, showSaldi: v })} />
        <ToggleRow label="Pagate"                  sub="Commesse chiuse"                   dotColor={TIPO_COLORS.pagata.bd}     value={draft.showPagate}     onChange={(v) => setDraft({ ...draft, showPagate: v })} />
