...
const ICO_FOLDER = (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>
);
const ICO_PAUSE = (
...
export default function OperatorDetailMobile({
  ...
  const ACTIONS_TOP = [
    { lbl: "Chiama", icon: ICO_PHONE, fn: onChiama, color: PAL.tealDark },
    { lbl: "Mappa",  icon: ICO_MAP,   fn: onMappa,  color: PAL.tealDark },
    { lbl: "Chat",   icon: ICO_CHAT,  fn: onChat,   color: PAL.tealDark },
    { lbl: "Foto",   icon: ICO_CAM,   fn: onFoto,   color: PAL.tealDark },
    { lbl: "Task",   icon: ICO_TASK,  fn: onTask,   color: PAL.tealDark },
  ];
  const ACTIONS_BOT = [
  ...
  // Per Marco: 2 completati / 1 in corso / 0 problemi (come da mockup)
  const completati = op.status === "attivo" ? 2 : (op.status === "pausa" ? 1 : 0);
  ...
  return (
    <div style={{ background: PAL.pageBg, minHeight: "100vh", paddingBottom: 100 }}>
      ...
      <div style={{
        ...
        {op.avatar_url ? (
          <img src={op.avatar_url} alt={op.name} style={{
            ...
            background: "#fff", border: "2px solid rgba(255,255,255,0.6)", objectFit: "cover" as any,
          }}/>
        ) : (
          <div style={{
            width: 42, height: 42, borderRadius: 999,
            ...
            fontSize: 14, fontWeight: 800, border: "2px solid rgba(255,255,255,0.6)",
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 2 }}>
            ...
            <span style={{ fontSize: 11, color: "rgba(255,255,255,0.92)", fontWeight: 600 }}>{s.text}</span>
          </div>
        </div>
      ...
      {/* AZIONI RAPIDE */}
      <div style={{ background: PAL.card, padding: "12px 14px 14px", borderBottom: `1px solid ${PAL.cardBorder}` }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: PAL.text, marginBottom: 10 }}>Azioni rapide</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 4, marginBottom: 6 }}>
          {ACTIONS_TOP.map(a => (
            <div key={a.lbl} onClick={a.fn} style={{ padding: "6px 4px", textAlign: "center" as any, cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 4, color: a.color }}>{a.icon}</div>
      ...
      {op.current_job && (
        <div style={{ background: PAL.card, padding: "14px 14px", borderBottom: `1px solid ${PAL.cardBorder}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: PAL.textSub }}>Lavoro attuale</div>
            <span style={{ color: PAL.textSub, cursor: "pointer" }}>{ICO_CHEV_R}</span>
          </div>
          <div style={{ fontSize: 16, fontWeight: 800, color: PAL.tealDark, marginTop: 4 }}>{op.current_job}</div>
          {op.commessa_code && (
          ...
          <div style={{ fontSize: 10, color: PAL.textSub, fontWeight: 600, marginTop: 12 }}>Iniziato alle 08:30</div>

          {/* Timer + Pausa/Stop INLINE */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4, gap: 8 }}>
            <div style={{ fontSize: 32, fontWeight: 800, color: PAL.text, letterSpacing: -0.8, lineHeight: 1, whiteSpace: "nowrap" as any }}>
              {timerBig}
            </div>
        ...
        <div style={{ background: PAL.card, padding: "14px 14px", borderBottom: `1px solid ${PAL.cardBorder}` }}>
          ...
          <div style={{ position: "relative", paddingLeft: 26 }}>
            ...
            {timeline.map(ev => {
              ...
              return (
                <div key={ev.id} style={{ display: "flex", alignItems: "flex-start", gap: 12, paddingBottom: 12, position: "relative" }}>
                  <span style={{
                    position: "absolute", left: -25, top: 4,
                    width: 12, height: 12, borderRadius: 999,
                    background: isPrev ? "#fff" : color,
                    border: isPrev ? `2px dashed ${color}` : `2px solid #fff`,
                  ...
                  {ev.has_photo && (
                    <div style={{ width: 38, height: 28, borderRadius: 4, background: "linear-gradient(135deg, #6B7280, #4B5563)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>📷</div>
                  )}
                </div>
              );
            })}
      ...
      <div style={{ background: PAL.card, padding: "14px 14px", borderBottom: `1px solid ${PAL.cardBorder}` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
          ...
          <Stat n={inCorso}    lbl="In corso"   color={PAL.viaggioDot} bg={PAL.viaggioBg} />
