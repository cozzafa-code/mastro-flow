        ...
        <div style={{ padding: "16px 16px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {(() => {
            ...
            if (_faseDb === "sopralluogo") {
              ...
              primaryAction = () => { console.log("[v-event] dispatch open-ordini", { id: selectedCM?.id, fase: selectedCM?.fase }); window.dispatchEvent(new CustomEvent("mastro:open-ordini", { detail: { commessa: selectedCM } })); };
                          ...
                          <div style={{ background: "#F8FBFB", border: "1.5px solid #C8E4E4", borderRadius: 10, padding: 12, marginBottom: 14 }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
                              {giorni.map((d, i) => {
                                ...
                                const disabled = weekend || isPast;
                                return (
                                  <div key={i} onClick={() => { if (!disabled) setAvviaFormData((p: any) => ({ ...(p || {}), data: iso })); }} style={{ minHeight: 70, padding: 6, borderRadius: 8, cursor: disabled ? "not-allowed" : "pointer", background: selezionato ? "#28A0A0" : weekend ? "#F1F5F9" : "#fff", border: "2px solid " + (selezionato ? "#1a6b6b" : weekend ? "#E2E8F0" : "#C8E4E4"), opacity: isPast ? 0.4 : 1, display: "flex" as any, flexDirection: "column" as any, justifyContent: "space-between", alignItems: "center" }}>
                                    {!weekend && (<>
                                      <div style={{ fontSize: 9, fontWeight: 800, color: selezionato ? "#fff" : "#6A8484" }}>{vani}/{CAPACITA}</div>
                                      <div style={{ width: "100%" as any, height: 6, background: selezionato ? "rgba(255,255,255,0.3)" : "#E2E8F0", borderRadius: 3, overflow: "hidden" as any }}>
                                        <div style={{ height: "100%" as any, width: Math.min(100, pct) + "%", background: selezionato ? "#fff" : colore(pct) }} />
