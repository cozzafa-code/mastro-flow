...
export default function CMDetailPanel() {
  ...
  if (showCadDraw) {
    ...
    if (prevWorkspace) {
      ...
      const chipPw = (on) => ({ padding: "7px 12px", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer", background: on ? `${L.primary}15` : L.surface, color: on ? L.primary : L.sub, border: `1.5px solid ${on ? L.primary : L.border}` });
      const chipPwGrn = (on) => ({ padding: "7px 12px", borderRadius: 8, fontSize: 11, fontWeight: 800, cursor: "pointer", background: on ? `${L.green}15` : L.surface, color: on ? L.green : L.sub, border: `1.5px solid ${on ? L.green : L.border}` });
      const inputPw = { width: "100%", padding: "10px", borderRadius: 8, border: `1px solid ${L.border}`, fontSize: 13, fontFamily: "inherit", boxSizing: "border-box" as any, background: L.surface };
      ...
      return (
        <div style={{ paddingBottom: 80 }}>
          {/* Header sticky */}
          <div style={{ background: L.primaryCont || "#1A1A1C", padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 10 }}>
            ...
            <div style={{ background: L.primary, padding: "5px 10px", borderRadius: 8, fontSize: 12, fontWeight: 900, color: "#fff", flexShrink: 0 }}>€{pwFmt(pwTotale)}</div>
          </div>
          ...
          {prevTab === "sopralluogo" && (
            <div style={{ padding: "0 12px 20px" }}>
              <div style={{ padding: 14, background: `${"#3b7fe0"}10`, borderRadius: 12, marginBottom: 14, border: `1px solid ${"#3b7fe0"}20` }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: "#3b7fe0" }}><I d={ICO.clipboard} /> Report Sopralluogo</div>
                  <div style={{ fontSize: 10, color: L.sub, background: L.surface, padding: "3px 8px", borderRadius: 6, fontWeight: 700 }}>R{pwRilievi.length} · {pwRilievi[0]?.data || "·"}</div>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6, marginBottom: 14 }}>
                {[
                  ...
                  <div key={i} style={{ background: L.surface, borderRadius: 10, padding: "10px 6px", textAlign: "center", border: `1px solid ${L.border}` }}>
              ...
              {pwVani.map(v => {
                ...
                return (
                  <div key={v.id} style={{ background: L.surface, borderRadius: 12, padding: 12, marginBottom: 10, border: `1px solid ${L.border}`, borderLeft: `4px solid ${misOk ? L.green : L.amber}` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 800 }}>{v.nome || `Vano ${v.id}`}
                          ...
                          {v.parentId && <span style={{ fontSize: 9, background: `${L.amber}15`, color: L.amber, padding: "2px 6px", borderRadius: 4, marginLeft: 4 }}>MODIFICA</span>}
                        </div>
                        <div style={{ fontSize: 10, color: L.sub }}>{v.tipo || "F2A"} · {v.stanza || "·"} · {v.piano || "PT"} · {v.pezzi || 1}pz</div>
                      </div>
                      <span style={{ fontSize: 10, background: misOk ? `${L.green}15` : `${L.red}15`, color: misOk ? L.green : L.red, padding: "3px 8px", borderRadius: 6, fontWeight: 700, height: "fit-content" }}>{misOk ? `✓ ${nMis}` : `⏹+ ${nMis}/6`}</span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, marginBottom: 8 }}>
                      {[{ l: "Larg.", val: lv }, { l: "Alt.", val: hv }, { l: "mq", val: ((lv * hv) / 1000000).toFixed(2) }].map((m, mi) => (
                        <div key={mi} style={{ background: L.bg, borderRadius: 8, padding: 8, textAlign: "center" }}>
                    ...
                    {v.note && <div style={{ fontSize: 11, color: L.amber, fontWeight: 600, marginTop: 4 }}><I d={ICO.mapPin} /> {v.note}</div>}
                    ...
                    {v.pdfFornitore ? (
                      ...
                      <div style={{ marginTop: 8, padding: "6px 10px", borderRadius: 8, background: "#D0800808", border: "1px dashed #D0800840", fontSize: 10, color: L.sub, display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {(Array.isArray(v.foto) && v.foto.length > 0) && (
                      <div style={{ display: "flex", gap: 6, marginTop: 8, overflowX: "auto" }}>
                        {v.foto.map((f, fi) => (
                          <div key={fi} style={{ minWidth: 64, height: 64, borderRadius: 8, background: `${"#3b7fe0"}08`, border: `1px solid ${"#3b7fe0"}20`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    ...
                    {v.parentId && (() => {
                      ...
                      return (
                        <div style={{ marginTop: 8, padding: 8, background: `${"#6366f1"}08`, borderRadius: 8, border: `1px solid ${"#6366f1"}20` }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                      <div onClick={() => { setPrevTab("preventivo"); setEditingVanoId(v.id); }} style={{ flex: 1, padding: "6px 0", borderRadius: 6, textAlign: "center", fontSize: 10, fontWeight: 700, color: L.primary, background: `${L.primary}08`, cursor: "pointer", border: `1px solid ${L.primary}20` }}><I d={ICO.clipboard} /> Modifica nel preventivo</div>
                    </div>
                  </div>
                );
              })}
          ...
          {prevTab === "riepilogo" && (
            <div style={{ padding: "0 12px 20px" }}>
              <div style={{ background: L.primaryCont || "#1A1A1C", borderRadius: 12, padding: 16, marginBottom: 12, color: "#fff" }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  {pwDetrObj && pwDetrObj.perc > 0 && (<div style={{ background: `${L.green}30`, borderRadius: 8, padding: "6px 10px", textAlign: "right" as any }}><div style={{ fontSize: 9, color: "#ffffffa0" }}>{pwDetrObj.l}</div><div style={{ fontSize: 14, fontWeight: 900, color: "#4ade80" }}>-€{pwFmt(pwDetraibile)}</div></div>)}
                </div>
              ...
              <div style={{ background: L.surface, borderRadius: 12, padding: 14, marginTop: 14, border: `1px solid ${L.border}` }}>
                ...
                {pwDetrObj && pwDetrObj.perc > 0 && (<>
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, padding: "8px 10px", background: `${L.green}10`, borderRadius: 8 }}>
              ...
              {c.notePreventivo && (
                <div style={{ marginTop: 12, padding: 12, background: L.bg, borderRadius: 10, fontSize: 10, color: L.sub, lineHeight: 1.8 }}>
              ...
              <div style={{ marginTop: 16, display: "flex", gap: 8, marginBottom: 8 }}>
                <button onClick={() => generaPreventivoPDF(c)} style={{ flex: 1, padding: 14, borderRadius: 10, background: `${L.primary}10`, color: L.primary, border: `1.5px solid ${L.primary}`, fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}><I d={ICO.fileText} /> PDF</button>
                <button onClick={() => generaPreventivoCondivisibile(c)} style={{ flex: 1, padding: 14, borderRadius: 10, background: L.surface, color: L.sub, border: `1.5px solid ${L.border}`, fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}><I d={ICO.eye} /> Anteprima</button>
              </div>
              ...
              {c.preventivoInviato && faseIndex(c.fase) < faseIndex("conferma") && (
                <button onClick={() => { setFaseTo(c.id, "conferma"); setPrevWorkspace(false); setCcDone(null); }} style={{ width: "100%", padding: 16, borderRadius: 12, border: "none", background: L.primary, color: "#fff", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", marginTop: 8 }}><I d={ICO.edit} />€+ AVANTI → Conferma ordine</button>
              )}
          ...
          {prevTab === "cad" && (
            <div style={{ padding: 16 }}>
              <div style={{ background: L.surface, borderRadius: 14, border: `1px solid ${L.border}`, padding: 20, textAlign: "center" }}>
                ...
                <button
                  ...
                  style={{ width: "100%", padding: 16, borderRadius: 12, border: "none", background: L.primary, color: "#fff", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>
                  ✓+ Apri disegnatore
                </button>
                {c.cadData && (
          ...
          {prevTab === "importa" && (
            <div style={{ padding: "0 12px 20px" }}>
              <div style={{ padding: 16, background: `${"#3b7fe0"}08`, borderRadius: 12, marginBottom: 12, border: `1px solid ${"#3b7fe0"}20` }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                <div style={{ flex: 1, padding: 20, background: L.surface, borderRadius: 12, border: `2px dashed ${"#3b7fe0"}40`, textAlign: "center", cursor: "pointer" }}>
                ...
                <div style={{ flex: 1, padding: 20, background: L.surface, borderRadius: 12, border: `2px dashed ${L.primary}40`, textAlign: "center", cursor: "pointer" }}>
              ...
              <div style={{ background: L.surface, borderRadius: 12, padding: 14, border: `1px solid ${L.border}`, marginBottom: 12 }}>
              ...
              <div style={{ padding: 12, background: `${L.green}08`, borderRadius: 10, border: `1px solid ${L.green}20`, fontSize: 11, color: L.green, fontWeight: 600, textAlign: "center" }}>
    ...
    const tipoColRil = tipoRil === "definitiva" ? L.green : tipoRil === "modifica" ? L.amber : "#3b7fe0";
              ...
              return (
                <div key={v.id} style={{ ...S.card, marginBottom: 10, overflow: "hidden", border: `1.5px solid ${hB ? L.red : L.border}` }}>
                  <div onClick={() => togB(v.id)} style={{ padding: "11px 14px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", background: hB ? "#ffdad6" : L.surface }}>
                    ...
                    <div style={{ width: 24, height: 24, borderRadius: 6, border: `2px solid ${hB ? L.red : L.border}`, background: hB ? L.red : "transparent", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 14 }}>
                      {hB ? "✓" : ""}
                    </div>
                  </div>
                  {hB && (
    ...
    return (
      <div style={{ paddingBottom: 80 }}>
        {/* Header rilievo */}
        <div style={{ ...S.header }}>
          <div onClick={() => { setSelectedRilievo(null); setCmSubTab("rilievi"); }} style={{ cursor: "pointer", padding: "8px 12px", borderRadius: 8, background: L.bg, border: `1px solid ${L.border}`, display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 700, color: L.text }}><Ico d={ICO.back} s={16} c={L.text} /> Indietro</div>
          <div style={{ flex: 1 }}>
          ...
          <div onClick={() => setShowRiepilogo(true)} style={{ padding: "6px 10px", borderRadius: 6, background: L.amberBg, cursor: "pointer", marginLeft: 6 }}>
            <span style={{ fontSize: 14 }}><I d={ICO.clipboard} /></span>
          </div>
          <div onClick={exportPDF} style={{ padding: "6px 10px", borderRadius: 6, background: "#ffdad6", cursor: "pointer" }}>
        ...
        {/* Banner rilievo info */}
        {r?.motivoModifica && (
          <div style={{ margin: "4px 16px 0", padding: "8px 12px", background: "#fff7ed", borderRadius: 8, border: `1px solid ${L.amber}30`, fontSize: 12, color: L.amber }}>
        ...
        <div style={{ padding: "8px 16px", display: "flex", gap: 6, flexWrap: "wrap" }}>
          {c.tipo === "riparazione" && <span style={S.badge("#fff7ed", L.amber)}><I d={ICO.wrench} /> Riparazione</span>}
          {c.tipo === "nuova" && <span style={S.badge("#d1fae5", L.green)}>Nuova</span>}
          {c.sistema && <span style={S.badge("#dbeafe", "#3b7fe0")}>{c.sistema}</span>}
          {c.difficoltaSalita && <span style={S.badge(c.difficoltaSalita === "facile" ? "#d1fae5" : c.difficoltaSalita === "media" ? "#fff7ed" : "#ffdad6", c.difficoltaSalita === "facile" ? L.green : c.difficoltaSalita === "media" ? L.amber : L.red)}>Salita: {c.difficoltaSalita}</span>}
          {c.mezzoSalita && <span style={S.badge("#6366f1"Lt, "#6366f1")}>{c.mezzoSalita}</span>}
        ...
        {c.note && <div style={{ padding: "0 16px", marginBottom: 6 }}><div style={{ padding: "8px 12px", borderRadius: 8, background: L.surface, border: `1px solid ${L.border}`, fontSize: 12, color: L.sub, lineHeight: 1.4 }}><I d={ICO.fileText} /> {c.note}</div></div>}
        ...
        {(() => {
          ...
          return (
            <div style={{ margin: "8px 16px 4px" }}>
              ...
              {/* Current action */}
              {curCC && (
                <div style={{ background: L.surface, borderRadius: 12, border: `1px solid ${L.border}`, padding: "12px 14px" }}>
                  ...
                  {curCC.id === "preventivo" && (
                    <div>
                      ...
                      {/* Totale rapido */}
                      <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 10px", background: `${L.primary}10`, borderRadius: 8, marginBottom: 10 }}>
                      ...
                      {/* BOTTONE PRINCIPALE */}
                      <button onClick={() => { setPrevWorkspace(true); setPrevTab("preventivo"); setEditingVanoId(null); }} style={{ width: "100%", padding: 16, borderRadius: 12, border: "none", background: L.primary, color: "#fff", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", marginBottom: 8 }}><I d={ICO.clipboard} /> APRI PREVENTIVO →</button>
                      ...
                      {firmaStep === 0 ? (
                        ...
                        <div>
                          ...
                          <div style={{ marginBottom: 12 }}>
                            ...
                            }} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", background: L.primary, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", marginBottom: 8 }}>
                              Invia link firma al cliente
                            </button>
                            {firmaLinkCopiato && (
                              <div style={{ background: "#F0FDF9", borderRadius: 8, padding: "8px 12px", fontSize: 11, color: L.green, fontWeight: 600, textAlign: "center" }}>
                          ...
                          <button onClick={() => { const inp = document.createElement("input"); inp.type = "file"; inp.accept = "application/pdf,image/*"; inp.onchange = (ev: any) => { const f = ev.target.files?.[0]; if (!f) return; setFirmaFileName(f.name); const r = new FileReader(); r.onload = (e) => setFirmaFileUrl(e.target?.result as string); r.readAsDataURL(f); }; inp.click(); }} style={{ width: "100%", padding: 14, borderRadius: 10, border: `2px dashed ${L.primary}`, background: `${L.primary}08`, color: L.primary, fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}><I d={ICO.download} /> CARICA FIRMATO</button>
                        </div>
                      ) : (
                        <div>
                  ...
                  {curCC.id === "ordini" && (
                    <div>
                      ...
                      <div style={{ background: L.bg, borderRadius: 8, padding: 8, marginBottom: 8, maxHeight: 120, overflow: "auto" }}>
                        {vaniCC.map((v, vi) => (
                        ...
                        <div style={{ marginBottom: 8 }}>
                          ...
                          <div style={{ display: "flex", gap: 4, marginBottom: 6, flexWrap: "wrap" as any }}>
                            {[30, 40, 50, 60, 100].map(p => (
                              <div key={p} onClick={() => setFattPerc(p)} style={{ padding: "6px 10px", borderRadius: 8, cursor: "pointer", fontSize: 10, fontWeight: 800, background: fattPerc === p ? L.primary : L.surface, color: fattPerc === p ? "#fff" : L.text, border: `1.5px solid ${fattPerc === p ? L.primary : L.border}` }}>
                                {p === 100 ? "Unica" : p + "%"}
                              </div>
                            ))}
                          </div>
                          <button onClick={() => { setAccontoImporto(String(Math.round(totIvaCC * fattPerc / 100))); setShowAccontoModal(true); }} style={{ width: "100%", padding: 11, borderRadius: 8, border: `1px solid ${L.primary}`, background: `${L.primary}08`, color: L.primary, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", marginBottom: 8 }}><I d={ICO.euro} /> Crea fattura €{fmtCC(Math.round(totIvaCC * fattPerc / 100))}</button>

                          {/* · MODAL IMPORTO ACCONTO · */}
                          {showAccontoModal && (
                            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
                              <div style={{ background: L.surface, borderRadius: 16, padding: 24, width: "100%", maxWidth: 360, boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
                                ...
                                <input
                                  ...
                                  style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: `1.5px solid ${L.primary}`, fontSize: 18, fontWeight: 800, color: L.primary, background: L.bg, fontFamily: "inherit", marginBottom: 6, boxSizing: "border-box" as any, outline: "none" }}
                                  autoFocus
                                />
                                ...
                                <div style={{ display: "flex", gap: 8 }}>
                                  <button onClick={() => setShowAccontoModal(false)} style={{ flex: 1, padding: 11, borderRadius: 10, border: `1px solid ${L.border}`, background: L.surface, color: L.sub, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Annulla</button>
                                  <button onClick={() => {
                                    ...
                                    setTimeout(() => setCcDone(null), 3000);
                                  }} style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: L.primary, color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>✓ CREA FATTURA €{fmtCC(parseFloat(accontoImporto) || 0)}</button>
                                </div>
                              </div>
                            </div>
                          )}
                      ...
                      <button onClick={() => setShowOrdinePreview(true)} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", background: L.primary, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}><I d={ICO.package} /> CREA ORDINE FORNITORE →</button>
                        ...
                        return (
                          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 9999, display: "flex", alignItems: "flex-end", justifyContent: "center", padding: "0" }}>
                            <div style={{ background: L.surface, borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 520, maxHeight: "90vh", overflow: "auto", padding: 20, boxShadow: "0 -8px 40px rgba(0,0,0,0.25)" }}>
                              ...
                              {/* Info fornitore */}
                              <div style={{ background: L.bg, borderRadius: 10, padding: 10, marginBottom: 12 }}>
                              ...
                              {/* Totali */}
                              <div style={{ background: L.bg, borderRadius: 10, padding: 10, marginBottom: 16 }}>
                              ...
                              <div style={{ marginBottom: 12 }}>
                                ...
                                <textarea value={noteOrdine} onChange={e => setNoteOrdine(e.target.value)} placeholder="Consegna urgente, RAL, riferimento..." rows={2} style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: `1px solid ${L.border}`, background: L.bg, color: L.text, fontSize: 11, resize: "none" as any, boxSizing: "border-box" as any, fontFamily: "inherit" }} />
                              </div>
                              {/* Bottoni */}
                              <div style={{ display: "flex", gap: 8 }}>
                                <button onClick={() => setShowOrdinePreview(false)} style={{ flex: 1, padding: 13, borderRadius: 10, border: `1px solid ${L.border}`, background: L.surface, color: L.sub, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Annulla</button>
                                <button onClick={() => {
                                  ...
                                  setTimeout(() => setCcDone(null), 3000);
                                }} style={{ flex: 2, padding: 13, borderRadius: 10, border: "none", background: L.primary, color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>
                  ...
                  {curCC.id === "produzione" && (
                    <div>
                      {ordCC.length > 0 && (
                        <div style={{ background: L.bg, borderRadius: 8, padding: 8, marginBottom: 8 }}>
                          {ordCC.map((o, oi) => (
                      ...
                      {!ordConfCC ? (
                        ...
                        <div style={{ background: "#34c75912", borderRadius: 10, padding: 12, border: "1px solid #34c75930" }}>
                          ...
                          <div style={{ display: "flex", gap: 8 }}>
                            <button onClick={() => setCcConfirm(null)} style={{ flex: 1, padding: 11, borderRadius: 10, border: `1px solid ${L.border}`, background: L.surface, color: L.sub, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Annulla</button>
                    ...
                    <div>
                      {!montFormOpen ? (
                        <div>
                          ...
                          <button onClick={() => { setMontFormOpen(true); setMontGiorni(1); setMontFormData({ data: "", orario: "08:00", durata: "giornata", squadraId: squadreDB[0]?.id || "", note: "" }); }} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", background: L.primary, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}><I d={ICO.wrench} /> PIANIFICA MONTAGGIO →</button>
                        </div>
                      ) : (
                        <div style={{ background: L.bg, borderRadius: 10, padding: 10, border: `1px solid ${L.border}` }}>
                          <input type="date" value={montFormData.data} onChange={e => { setMontFormData(p => ({ ...p, data: e.target.value })); setWorkWeekend(null); }} style={{ width: "100%", padding: 10, borderRadius: 8, border: `1px solid ${L.border}`, fontSize: 14, fontFamily: "inherit", boxSizing: "border-box" as any, marginBottom: 6 }} />
                          ...
                          {(() => {
                            ...
                            return (
                              <div style={{ marginBottom: 8, borderRadius: 8, border: '1px solid ' + L.border, overflow: 'hidden', background: L.surface }}>
                          ...
                          <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                            <select value={montFormData.orario} onChange={e => setMontFormData(p => ({ ...p, orario: e.target.value }))} style={{ flex: 1, padding: 10, borderRadius: 8, border: `1px solid ${L.border}`, fontSize: 12, fontFamily: "inherit" }}>
                          ...
                          {squadreDB.length > 0 && <div style={{ display: "flex", gap: 4, marginBottom: 6, flexWrap: "wrap" as any }}>{squadreDB.map(sq => (
                            <div key={sq.id} onClick={() => setMontFormData(p => ({ ...p, squadraId: sq.id }))} style={{ padding: "6px 12px", borderRadius: 8, cursor: "pointer", fontSize: 11, fontWeight: 700, background: montFormData.squadraId === sq.id ? L.primary : L.surface, color: montFormData.squadraId === sq.id ? "#fff" : L.text, border: `1px solid ${montFormData.squadraId === sq.id ? L.primary : L.border}` }}>{sq.nome || sq.id}</div>
                          ))}</div>}
                          <div style={{ display: "flex", gap: 6 }}>
                            <button onClick={() => setMontFormOpen(false)} style={{ flex: 1, padding: 11, borderRadius: 10, border: `1px solid ${L.border}`, background: L.surface, color: L.sub, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Annulla</button>
                            <button onClick={() => {
                  ...
                  {curCC.id === "chiusura" && (() => {
                    ...
                    <div>
                      ...
                      {!saldoFatCC && restoCC > 0 && (
                        ccConfirm !== "saldo" ? (
                          <button onClick={() => setCcConfirm("saldo")} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", background: L.primary, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}><I d={ICO.euro} /> FATTURA SALDO €{fmtCC(restoCC)} →</button>
                        ) : (
                          <div style={{ background: L.primary + "10", borderRadius: 10, padding: 12, border: `1px solid ${L.primary}30` }}>
                            <div style={{ fontSize: 13, fontWeight: 800, color: L.primary, marginBottom: 4 }}>Fattura saldo €{fmtCC(restoCC)}</div>
                            <div style={{ display: "flex", gap: 8 }}>
                              <button onClick={() => setCcConfirm(null)} style={{ flex: 1, padding: 11, borderRadius: 10, border: `1px solid ${L.border}`, background: L.surface, color: L.sub, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Annulla</button>
                              <button onClick={() => { creaFattura(c, restoCC === totIvaCC ? "unica" : "saldo"); setCcConfirm(null); setCcDone("✓ Fattura saldo creata!"); setTimeout(() => setCcDone(null), 3000); }} style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: L.primary, color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>✓ CREA FATTURA</button>
                            </div>
                          </div>
                        )
                      )}
              ...
              {/* Totale */}
              {totPrevCC > 0 && (
                <div style={{ marginTop: 6, display: "flex", justifyContent: "space-between", padding: "8px 12px", background: L.surface, borderRadius: 8, border: `1px solid ${L.border}` }}>
          ...
          <div style={{ padding: "0 16px 14px" }}>
            ...
            {isStorico && (
              <div style={{ padding: "10px 14px", borderRadius: 10, marginBottom: 10, background: "#5856d610", border: "1.5px solid #5856d630", display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div onClick={() => { if (lastRilievo) setSelectedRilievo(lastRilievo); }} style={{ padding: "6px 12px", borderRadius: 8, background: L.primary, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
                ...
                <div key={v.id} onClick={() => { console.log("CLICK VANO", v?.id, v?.nome); setSelectedVano(v); setVanoStep(0); }}
                  ...
                  <div style={{ textAlign: "right", display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4 }}>
                    ...
                    {bloccato
                      ...
                      : <span style={S.badge("#fff7ed", L.amber)}><I d={ICO.alertTriangle} /> {nMisure} mis.</span>}
                  </div>
              ...
              return (
                <div key={v.id} style={{ ...S.card, marginBottom: 8 }}>
                  <div style={{ padding: "11px 14px", display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 8, flexShrink: 0, background: mis ? "#d1fae5" : blk ? "#ffdad6" : L.border, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>
                      {mis ? "✓" : blk ? "+" : "📐"}
                    </div>
                    <div style={{ flex: 1 }}>
        ...
        <div style={{ padding: "0 16px", marginBottom: 8, display: "flex", gap: 8 }}>
          ...
          {problemi.filter(p => p.commessaId === c.id).length > 0 && (
            <button onClick={() => { setShowProblemiView(true); }} style={{ padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${L.border}`, background: L.surface, color: L.text, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: FF, display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {(c.allegati || []).map(a => (
                <div key={a.id} style={{ borderBottom: `1px solid ${L.bg}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px" }}>
                    ...
                    {/* Audio: play inline */}
                    {a.tipo === "vocale" && (
                      <div onClick={() => playAllegato(a.id)} style={{ padding: "3px 8px", borderRadius: 6, background: playingId === a.id ? "#ffdad6" : L.amberBg, fontSize: 10, fontWeight: 600, color: playingId === a.id ? L.red : L.primary, cursor: "pointer" }}>
                        {playingId === a.id ? "⏹ Stop" : "▶ Play"}
                      </div>
                    )}
                    {/* Video: open/close inline player */}
                    {a.tipo === "video" && a.dataUrl && (
                      <div onClick={() => setViewingVideoId(viewingVideoId === a.id ? null : a.id)} style={{ padding: "3px 8px", borderRadius: 6, background: viewingVideoId === a.id ? "#dbeafe" : L.amberBg, fontSize: 10, fontWeight: 600, color: viewingVideoId === a.id ? "#3b7fe0" : L.primary, cursor: "pointer" }}>
                        {viewingVideoId === a.id ? "✓ Chiudi" : "▶ Guarda"}
                      </div>
                    )}
                    {a.tipo === "video" && a.dataUrl && (
                      <a href={a.dataUrl} download={a.nome || "video.webm"} style={{ padding: "3px 8px", borderRadius: 6, background: L.bg, fontSize: 10, fontWeight: 600, color: L.sub, cursor: "pointer", textDecoration: "none" }}>🔗</a>
                    )}
                    {/* Photo: toggle inline preview */}
                    {a.tipo === "foto" && a.dataUrl && (
                      <div onClick={() => setViewingPhotoId(viewingPhotoId === a.id ? null : a.id)} style={{ padding: "3px 8px", borderRadius: 6, background: L.amberBg, fontSize: 10, fontWeight: 600, color: L.primary, cursor: "pointer" }}>
                    ...
                    {a.tipo === "file" && a.dataUrl && <a href={a.dataUrl} download={a.nome} style={{ padding: "3px 8px", borderRadius: 6, background: L.amberBg, fontSize: 10, fontWeight: 600, color: L.primary, cursor: "pointer", textDecoration: "none" }}><I d={ICO.folder} /> Apri</a>}
                    ...
                    {a.tipo === "firma" && a.dataUrl && <a href={a.dataUrl} download={a.nome} style={{ padding: "3px 8px", borderRadius: 6, background: L.bg, fontSize: 10, fontWeight: 600, color: L.sub, cursor: "pointer", textDecoration: "none" }}>🔗</a>}
                    {/* Delete */}
            ...
            <div style={{ background: L.bg, borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 580, maxHeight: "88vh", overflowY: "auto", padding: "20px 16px 32px" }}>
                ...
                <div style={{ background: "#E8F5E9", borderRadius: 12, padding: "12px 14px", marginBottom: 14, border: "1px solid #A5D6A7" }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    {c.email && (
                      ...
                      }} style={{ flex: 1, padding: "8px", borderRadius: 8, background: "#3b7fe0", border: "none", color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                        📧 Email
                      </button>
                    )}
                  </div>
              ...
              <div style={{ display: "flex", flexDirection: "column" as const, gap: 8, marginBottom: 16 }}>
                ...
                <button
                  ...
                  style={{ width: "100%", padding: 14, borderRadius: 12, border: `1.5px solid ${L.primary}`, background: `${L.primary}10`, color: L.primary, fontSize: 14, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
                >
                ...
                <button
                  ...
                  style={{ width: "100%", padding: 14, borderRadius: 12, border: `1.5px solid ${L.green}`, background: `${L.green}10`, color: L.green, fontSize: 14, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
                >
              ...
              {fascicoliStorico.length > 0 && (
                <div>
                  ...
                  {fascicoliStorico.map((fs) => {
                    ...
                    return (
                      <div key={fs.token} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: L.surface, borderRadius: 10, marginBottom: 6, border: `1px solid ${L.border}`, opacity: scaduto ? 0.5 : 1 }}>
                        ...
                        {!scaduto && (
                          <>
                            <button onClick={() => { navigator.clipboard.writeText(link); }} style={{ padding: "4px 8px", borderRadius: 6, border: `1px solid ${L.border}`, background: L.bg, fontSize: 10, cursor: "pointer", color: L.text }}>Copia</button>
                            <button onClick={async () => { await revocaFascicolo(fs.token); const s = await getFascicoliCommessa(c.id); setFascicoliStorico(s); }} style={{ padding: "4px 8px", borderRadius: 6, border: `1px solid ${L.red}30`, background: `${L.red}10`, fontSize: 10, cursor: "pointer", color: L.red }}>Revoca</button>
                          </>
                        )}
                      </div>
                    );
